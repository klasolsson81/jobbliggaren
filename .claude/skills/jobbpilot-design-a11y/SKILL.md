...
webaim.org/resources/contrastchecker. `--jp-text-tertiary` (darkened from
`#7C8AA0` in issue #296) clears AA on white — it is the demoted metadata tier
(timestamps, hints, IDs, dimmed dates), no longer body-text-blocked.
Placeholders still use `--jp-placeholder`, never tertiary.
