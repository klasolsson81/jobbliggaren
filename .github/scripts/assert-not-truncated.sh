...
#
# WHY IT IS ITS OWN FILE AND NOT FOUR LINES OF YAML. It was four lines of YAML,
...
if [ "$n" -ge "$LIMIT" ]; then
  fail "$FILE returned $n entries against a ceiling of $LIMIT -- the list may be truncated, and a truncated open-PR list is fail-OPEN (both open-PR guards dissolve for the PRs that fell off the end). Refusing to decide. Raise the ceiling."
fi
