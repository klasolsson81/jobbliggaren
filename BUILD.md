...
| PDF generation | QuestPDF | 2026.7.2 | Community v3.0 (source-available, non-copyleft). Eligible on the **revenue** ground — categories (1)/(6), USD 1M threshold — never on category (5) open-source, which requires an OSI-approved licence and ours is PolyForm Noncommercial (ADR 0072). Public-sector entities are ineligible regardless of revenue. *(This row said "ADR 0050-safe"; measured 2026-08-02, ADR 0050 is the deployment migration and carries zero licence content. No ADR adjudicates third-party dependency licences yet — see the pin comment in `Directory.Packages.props`.)*; `QuestPDF.Settings.License = LicenseType.Community` i startup |
