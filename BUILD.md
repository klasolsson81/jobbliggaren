...
| Form | React Hook Form + Zod (+ `@hookform/resolvers`) | RHF 7.x, Zod 4.x, resolvers 5.x | Schema-baserad validering. **`@hookform/resolvers` är adaptern mellan de två andra och har ingen egen förmåga** — den låg top-level i `dependencies` och i låsfilen sedan `18f5a0ed` men var **oanvänd** fram till #1514, då `new-application-form` och `add-follow-up-form` flyttade sina klientrefusals från handrullad `schema.safeParse` + `useState` till `zodResolver`. Raden namnger den nu i stället för att låta första användningen se ut som ett bibliotek utanför §3.1 (senior-cto-advisor 2026-08-27; CLAUDE.md §9.2/§12). **`{ raw: true }` är obligatoriskt** i den här kodbasen: utan det får `handleSubmit` schemats *transformerade* utdata, där ett tomt frivilligt fält blivit `undefined`, och `formData.set(k, undefined)` postar strängen `"undefined"` |
