...
| OTP-fält | input-otp (via shadcn/ui InputOTP) | 1.x | MIT, inga runtime-beroenden. Ritar sex rutor över ETT riktigt `<input>` (`autocomplete="one-time-code"`), så etikett, värde, inklistring och tabbstopp förblir ett element. Injicerar en `<style>` vid körning (kräver `style-src 'unsafe-inline'` eller `nonce`-prop). Klas-beslut 2026-09-24 ("Sex rutor, riv ADR-raden"); formen i ADR 0142 Page form, Amendment 2026-09-24 (9) |
...
| Form | React Hook Form + Zod (+ `@hookform/resolvers`) | RHF 7.x, Zod 4.x, resolvers 5.x | Schema-baserad validering. **`@hookform/resolvers` är adaptern mellan radens två andra namn** — den kom in i `e06675eb` (2026-05-08) i **samma commit som `react-hook-form` och `zod`**, alltså som en del av samma stack-beslut och aldrig som ett eget biblioteksval, men var **oanvänd** fram till #1514, då `new-application-form` och `add-follow-up-form` flyttade sina klientrefusals från handrullad `schema.safeParse` + `useState` till `zodResolver`. Raden namnger den nu i stället för att låta första användningen se ut som ett bibliotek utanför §3.1 (senior-cto-advisor 2026-08-27; CLAUDE.md §9.2/§12) |
