...
- **Backlog = GitHub Issues** (`area:`/`hotspot:`/`P0`–`P3`/lane `BE`·`FE`·
  ...
  does not disarm the gate (§6). It does **not** close issues — this file used to say
  it should, contradicting the next bullet, which has always assigned that duty to the
  owning session and is the one that is right (Klas, 2026-07-28: *"Jag kör ingen sådan
  loop — det ska CC göra"*; the babysitter prompt in playbook §8 never instructed it
  either). Side-track PRs you own are shepherded to green before new scope.
