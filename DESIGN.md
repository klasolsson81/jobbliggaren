...
- **Input/Select-höjd — samma två system:** `.jp-input` = **48px** (4 filer; `.jp-sortfield__select` är en egen fältklass och läser i stället `--jp-control-h`; `.jp-select`/`.jp-textarea` städades bort i #1073). shadcn `Input` = **44px** (ingen size-prop) och `SelectTrigger` = **44px** (`sm` 36px); de bär flest ytor (`ui/input` i 20 filer, `ui/select` i 2 produktionsfiler — 3 med testfilen; `.jp-btn`s 46 räknar tester med). Textarea bärs av `[data-slot="textarea"]`. Raden här sa tidigare bara 44px. Konsekvens värd att känna till: en `.jp-input` (48px) och en `.jp-btn` (44px) sida vid sida på samma rad linjerar INTE — para dem med `.jp-btn--field` (48px) när de ska läsas som en kontroll. Label alltid ovanför, hint under. **Inga beskrivande placeholder-exempel i sök/filter-fält** (Nielsen/WCAG-anti-pattern). Format-placeholder i auth-formulär OK (`namn@exempel.se` = syntax, ej exempelinnehåll)
