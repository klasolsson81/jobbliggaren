...
- **Knapphöjd — TVÅ system live, OCH ETT RATIFICERAT VÄRDE SOM INGET AV DEM HÅLLER.** **ADR 0038 §Beslut band knapp-höjd till 40px (sm 36)** och pekar ut `globals.css` `.jp-*`-komponentstorlekar som implementationsyta. Koden: `.jp-btn` = **44px** (`.jp-btn--sm` 36px) i 46 filer; shadcn `Button` = **40px** (`sm` 36, `lg` 44). Alltså följer shadcn-primitiven ADR:et och `.jp-btn` gör det inte — 44px är **oratificerad drift**, inte en andra standard. Radius 4px, transition 80ms, max EN `--primary` per skärm (ADR 0038). Se drift-noten under input-raden.
...
- **Input/Select-höjd — samma bild, samma drift.** **ADR 0038 band input-höjd till 44px (sm 40).** Koden: `.jp-input` = **48px** (4 filer) — oratificerad drift; `grep "48px" docs/decisions/` ger noll träffar. shadcn `Input` = **44px** (ingen size-prop) och `SelectTrigger` = **44px** (`sm` 36) följer ADR:et och bär flest ytor (`ui/input` i 20 filer, `ui/select` i 2 produktionsfiler — 3 med testfilen; `.jp-btn`s 46 räknar tester med). `.jp-sortfield__select` är en egen fältklass och läser `--jp-control-h`; `.jp-select`/`.jp-textarea` städades bort i #1073; textarea bärs av `[data-slot="textarea"]`. Konsekvens av driften: en `.jp-input` (48px) och en `.jp-btn` (44px) på samma rad linjerar INTE — `.jp-btn--field` (48px) parar dem tills driften är avgjord. Label alltid ovanför, hint under. **Inga beskrivande placeholder-exempel i sök/filter-fält** (Nielsen/WCAG-anti-pattern). Format-placeholder i auth-formulär OK (`namn@exempel.se` = syntax, ej exempelinnehåll)
- **DRIFT-NOT (öppen, Klas-beslut — #1095):** raderna ovan beskriver vad som *körs*, inte vad som är *beslutat*. ADR 0038 (Accepted 2026-05-16, Klas-GO) band 44/40; koden kör 48/44. Ingen senare amendment rör höjderna och ingen ADR nämner 48px. Korpusen är delad: ADR 0038, `jobbpilot-design-components/SKILL.md` och `references/variants-full.md` säger 44 (de två senare med orden "matches `.jp-input`", vilket alltså är falskt om koden); `references/tokens-full.md` och den här filen säger 48. **Tokenisera inte 48:an** — ett `--jp-field-h` skulle cementera den siffra som råkar vinna och flytta motsägelsen till ett tokennamn där ingen letar (samma form som density-systemet, retirerat 2026-07-26). Avgör 44 mot 48 först; 44 är dessutom WCAG 2.5.5-golvet ADR 0038 uttryckligen band.
