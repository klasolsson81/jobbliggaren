...
- **Knapphöjd — ett system håller det ratificerade värdet, det andra inte.** **ADR 0038 §Beslut band knapp-höjd till 40px (sm 36)** och pekar ut `globals.css` `.jp-*`-komponentstorlekar som implementationsyta. shadcn `Button` = **40px** (`sm` 36, `lg` 44) — följer ADR:et. `.jp-btn` = **44px** (`.jp-btn--sm` 36px) i 46 filer (42 exkl. tester) — avviker; se drift-noten nedan för varifrån den siffran kom. Radius `--jp-r-md` = **6px** (ADR 0052 Beslut 4: knappar 6px, inputs 4px — höjde golvet från ADR 0016/0037:s 4px). Transition **90ms** (`.jp-btn`; shadcn `Button` `duration-75` = 75ms). Max EN `--primary` per skärm (ADR 0038).
...
- **Input/Select-höjd — samma bild.** **ADR 0038 band input-höjd till 44px (sm 40).** shadcn `Input` = **44px** (ingen size-prop) och `SelectTrigger` = **44px** (`sm` 36) följer ADR:et och bär flest ytor (`ui/input` i 20 filer, `ui/select` i 2 produktionsfiler — 3 med testfilen). `.jp-input` = **48px** (4 filer) — avviker. `.jp-sortfield__select` är en egen fältklass och läser `--jp-control-h`; `.jp-select`/`.jp-textarea` städades bort i #1073; textarea bärs av `[data-slot="textarea"]` (`min-h-16`, ingen fast höjd). Konsekvens: en `.jp-input` (48px) och en `.jp-btn` (44px) på samma rad linjerar INTE — `.jp-btn--field` (48px) parar dem tills frågan är avgjord. Label alltid ovanför, hint under. **Inga beskrivande placeholder-exempel i sök/filter-fält** (Nielsen/WCAG-anti-pattern). Format-placeholder i auth-formulär OK (`namn@exempel.se` = syntax, ej exempelinnehåll)
- **DRIFT-NOT (öppen, Klas-beslut — #1095):** raderna ovan beskriver vad som *körs*, inte vad som är *protokollfört*. ADR 0038 (Accepted 2026-05-16, Klas-GO) band 44/40; koden kör 48/44. **Varifrån 48/44 kom, mätt:** `git log -L` på regelraderna visar `2e6af29b` (2026-05-19) — *"v3 `.jp-*`-system + header-shell (ADR 0052/0054)"*. ADR 0052 är Accepted med Klas-GO och dess header säger uttryckligen **"Amends: … ADR 0038"**. Men 0052:s Beslut-text nämner höjderna **noll gånger** (radien och typografin protokollfördes; höjderna är den enda v3-omkalibrering som aldrig skrevs ned). Så detta är inte hemlös drift — det är en **Klas-GO:d ändring utan protokollförd beslutsrad**. **Delad korpus:** ADR 0038, `jobbpilot-design-components/SKILL.md:61,119`, `references/variants-full.md:72` och `jobbpilot-design-principles/SKILL.md:153,243` säger ADR-värdena (flera med orden "matches `.jp-btn`/`.jp-input`", vilket är falskt mot koden — och `principles:243` är ett checklistesteg som körs varje design-agent-runda); `references/tokens-full.md` och den här filen beskriver koden. `globals.css:1894-1898` bär dessutom en normativ kommentar som säger "Ändra aldrig .jp-btn eller .jp-input till varandras höjd" — den föregriper #1095 och rättas där. **Tokenisera inte 48:an** — ett `--jp-field-h` skulle cementera den siffra som råkar vinna och flytta motsägelsen till ett tokennamn där ingen letar (samma form som density-systemet, retirerat 2026-07-26). Avgör värdet först. **A11y skiljer dem inte:** både 44 och 48 klarar en 44px-träffyta, och ADR 0038 nämner aldrig WCAG-träffytor (det värde som ligger *under* 44 är ADR:ets egen knapp-höjd, 40px).
