...
- **Hero-gradient (scoped undantag, ADR 0068):** `--jp-hero-gradient` (118° `#0B2A1E`→`#14503A`→`#1E6B4C`) ENBART på hero-banner-plattan/pagehero/landing-hero.
...
- **Inloggningsleverantörernas märken (scoped undantag, ADR 0142, Klas 2026-09-25):** en AKTIV provider-rad på `/logga-in` bär leverantörens officiella märke, oförändrat, i ägarens färger och med märkets egen gradient, 20px före radens text och `aria-hidden` (namnet kommer ur texten). Färgerna är varumärkets, inte våra: ingen `--jp-*`-token, aldrig återanvända utanför märket, ett enda hem: filen byte-identisk med leverantörens tillgång och med exakt en konsument. Raden förblir shadcn `Button variant="outline"` i appens typsnitt, text- och kantfärg; leverantörens knappvärden tas inte över och dess färg blir aldrig fyllning. Inaktiva rader bär inget märke. Undantaget gäller även §7 och AGENTS.md §5:s gradientförbud, bara inuti märket, och når ingen annan yta. 6a levererar Googles G; LinkedIns och GitHubs landar med sina leverantörer, pinnade på samma sätt.
