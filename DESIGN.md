...
- **Copy-täthet: det självklara förklaras inte** (Klas-direktiv 2026-09-24, "så få klick och info som
  möjligt"; #1003; ADR 0144). `design-reviewer` graderar överskott som **Major**, på samma sätt som
  hon graderar brist (ADR 0047):
  1. Ingen mening som säger vad h1, etikett eller knapp redan säger.
  2. En formulärsida bär högst EN ledtext, och bara när h1 + etikett + knapp inte redan säger vad
     som händer.
  3. Formathjälp (hint) bara när formatet inte är självklart. E-postadressens syntax är självklar
     och får ingen hint; organisationsnummer, datum i fritext och concept-id är inte självklara.
  4. Vad systemet gör härnäst skrivs bara där användaren inte kan sluta sig till det av knappen.
  5. Förklaringar ligger bakom ?-hjälp/expander-mönstret (#1003), aldrig inline som default.
  6. Lugnande meningar är förbjudna (en försäkran om vad vi inte gör, en dementi av ett krav som
     inte ställs): en yta som inte påstår något behöver inte dementera det.
  7. Rättsligt bärande text står kvar i sin minsta signerade form: den kortas bara med
     `security-auditor`s signatur och stryks aldrig. Vilka strängar det är: ADR 0144.
  Brist och överskott möts i samma mening: går uppgiften inte att slutföra utan att gissa när
  meningen stryks (ADR 0047), är fyndet en kortare mening eller en bättre etikett/knapp, aldrig
  gissningen.
