...
2. ✅ **Jobbkort Spara + Har-ansökt** *(large, backend+frontend)* — **LEVERERAD & PUSHAD 2026-05-23** (HEAD `1972f47`; 4 commits `c015918`/`4afc081`/`a187467`/`1972f47`). SavedJobAd-aggregat + `/api/v1/me/saved-job-ads` (GET/POST/DELETE); CreateApplicationFromJobAdCommand + `POST /api/v1/applications/from-job-ad/{jobAdId}` (ADR 0048 read-join, ingen snapshot); FE: SaveJobAdToggle + HarAnsoktButton i modal-footer + `/sparade`-sida + app-shell nav. ADR 0053-amend (lyft deferral) + ADR 0024-amend (cascade-rad). Pending Klas-GO tag-push `v0.2.58-dev` → migration auto-applied.
...
**Commits sedan föregående status (F6 P5 P2 — 4 commits, push-only, deploy pending Klas-GO):**
...
| (denna synk) | docs(sessions/current-work/steg-tracker) | F6 P5 P2 session-logg + current-work-statusflytt + steg-tracker (om relevant) |
