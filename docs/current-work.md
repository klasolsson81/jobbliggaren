...
**NÄSTA SESSION — återuppta C4 (architect-design LÅST; läs `docs/reviews/2026-05-18-td13-*` + ADR 0049 Beslut 5/Mekanik-not 1; architect agentId af92815b78c2fb817):** `resume_versions.content` (JSONB, sista kolumnen). **C4.0** empiriskt gate-test (probe-interceptor: ser interceptorn JSON-`string` [GRÖN, behåll JSON-VC] eller `ResumeContent`-objekt [RÖD, VC ut ur krypto-väg, interceptor äger transform — architect förutspår RÖD]) — kör FÖRST, blockerar C4.2; båda grenar förspecificerade (ingen ny design-runda). **C4.1** additiv migration `content_enc text NULL` + dual-property-mappning (parallellt, gren-oberoende; db-migration-writer obligatorisk). **C4.2** `EncryptedFieldRegistry` Form B (String|JsonSerializedVo-shape) + interceptor-VO-gren + ResumeVersion ägar-resolution (write skugg-FK `ResumeId`→Resume.JobSeekerId; read via ICurrentDataOwner) + gren-aktivering per C4.0. **C4.3** markörer (CreateResume/UpdateMasterContent/GetResumeById — verifiera DeleteResume*-handlers) + arch-test-spärrar. **C4.4** testsvit. Sedan **C5** BackfillFieldEncryptionJob (Hangfire-chassi + fitness `COUNT(*) WHERE content_enc IS NULL=0`; cutover-flipp = **Klas-STOPP** fitness-ratchet ADR 0045). **C6** crypto-erasure-hook i `AccountHardDeleter.HardDeleteAccountAsync` — anropa `IUserDataKeyStore.DeleteDataKeysAsync` INOM hard-delete-transaktionen (security C2/C3-överlämning, Art. 17-atomicitet) + CryptoErasure/SecurityLogging-tester. **STOPP I**-rapport (full svit + SQL-bevis taxonomi-sök intakt). **STOPP V** = Klas-GO tag-deploy v0.2.x-dev + rök-verify + raw_payload-generated-cols/taxonomi-regress + TD-13→tech-debt-archive (§9.7) + FAS 4-startprompt. **Senare (Klas-STOPP):** Beslut 5 Steg D drop `content` jsonb (destruktiv, separat commit, prod-verifiering).
