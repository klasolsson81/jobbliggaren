...
**Status:** **F6 P5 PUNKT 1 SNAPSHOT-RETENTION LEVERERAD 2026-05-23 (working-tree, ej committat ännu — väntar Klas-GO för tag-push `v0.2.57-dev`).** ADR 0032-amendment 2026-05-23 + ADR 0062-amendment 2026-05-23 (snapshot-miss-cleanup + ExpiresAt-cron + ApplyCriteria Status=Active SPOT-filter + post-archive circuit-breaker). Defense-in-depth-arkitektur enligt senior-cto-advisor-dom (agentId `a8e277380b446bb02`) + dotnet-architect-design (agentId `a10f8271fe298246c`) + tilläggsrond H1 post-archive circuit-breaker (agentId `acfe2963371fde555`). **Reviews:** code-reviewer 0 Block/0 Major/2 Minor accept-som-är (agentId `a82b9f511ec54889b`); security-auditor 0 Critical/0 Block, H1 in-block-fixad + H2 in-block-fixad (agentId `a419beb4d87e8a46e`). **Tester:** Domain 399 / Application 546 (+4 nya retention-tester + 4 nya snapshot-floor-tester + 6 nya RetainPlatsbanken-tester) / Architecture 78 (+5 nya `JobAdRetentionLayerTests`). Worker.IntegrationTests + Api.IntegrationTests körs i CI (Docker ej igång lokalt). **Korpus förväntad konvergens:** 56k → ~40k aktiva över ~72h post-deploy (N=3 konsekutiva snapshot-misses krävs). **`/jobb`-UX-impact:** räkne-droppet synligt samma deploy (Status=Active SPOT-filter) per CTO Variant 1 (filter+retention samma release). **Pending Klas-GO:** commit + push + tag-push `v0.2.57-dev` → dev-deploy → post-deploy-verifiering korpus-konvergens via `/api/v1/job-ads?pageSize=1 totalCount`. Session-logg pending.
...
1. **Snapshot-retention** *(small, backend-only)* — `sync-platsbanken-snapshot` är idag idempotent UPSERT utan expirerings-pass. Tillägg: markera annonser som inte längre finns i ny snapshot som `Status=Closed`/soft-delete + dedikerat Hangfire-jobb för `ExpiresAt < now` → `MarkExpired()`. **Korpus förväntad: 56k → ~46k** (matchar Platsbanken). Förutsättning för meningsfulla räkningar i punkt 2+3+4. ADR 0032 §1-amend; CTO-rond vid sessionsstart för retention-strategi (Variant: snapshot-miss-cleanup / Hangfire-expiry-job / båda).
2. **Jobbkort Spara + Har-ansökt** *(large, backend+frontend)* — F6 P4b `SavedJobAd`-aggregat avblockerad sedan 2026-05-20; "Har ansökt"-flöde = `CreateApplicationFromJobAdCommand` med JobAd-snapshot (ADR 0048 read-join-mönster). Frontend: jobbkort-toggles i `/jobb` + ADR 0053 modal-footer. **Kräver ADR 0053-amend** (lyfta "Spara/Har-ansökt fas-deferrade till Fas 4"-deferral 2026-05-19); ev. ny ADR för Application-from-JobAd snapshot-strategi. CTO-rond: snapshot-vid-skapande vs join-vid-läs.
3. **Landing live-stats** *(medium, backend endpoint + frontend swap)* — ny `GET /api/v1/landing/stats` (cache:ad — Redis / Hangfire-cron / materialiserad PG-vy), frontend swappar `getLandingStats()` per ADR 0056-utbytespunkt. **Perf-kritiskt** (publik landingpage, ingen auth). Beror på punkt 1 för korrekta antal. ADR 0056-amend; CTO-rond: cache-mekanism.
4. **Översiktssida `/oversikt`** *(large, frontend+ev. backend)* — handover-källa `docs/jobbpilot-v3-bundle/JobbPilot/handoff-oversikt/HANDOVER-oversikt.md` (Klas-godkänd 2026-05-23). Tre sektioner: Title+I dag / Notiser / Sammanfattning. Mockdata-tillåtelse för fält där BE saknas (HANDOVER §0). **Stänger TD-82**. Återanvänder punkt 3:s cache-mönster. CTO-rond: data-mappning riktigt-vs-mock (HANDOVER §3).
...
**Commits sedan föregående status (2 commits + 1 deploy):**
...
| (denna synk) | docs(sessions/current-work) | F6 P4 deploy-verify session-logg + current-work-statusflytt |
...
**Senast uppdaterad:** 2026-05-21 (F6 P4 FTS-skifte LEVERERAD & PUSHAD — PostgreSQL FTS-hybrid + Infrastructure-query-port `IJobAdSearchQuery`, ADR 0062 NY + 0061/0039-amend, `build`-CI grön; session-end docs-synk denna commit)
**HEAD:** `95bcb74` (origin/main — `docs(reviews)` sök-spinner design-review; `57feaac` web sök-skeleton; `b70b548` ADR 0062 + amends + reviews; `1eec29c` lager-refaktor `IJobAdSearchQuery`; `1b04765` FTS search_vector + GIN-index; session-end docs-synk-commit ovanpå)
**Deploy:** `v0.2.55-dev` LIVE på dev-backend (`/api/ready` 200) — F6 P4 sök-infrastruktur-fix. **F6 P4 FTS-skifte EJ DEPLOYAD:** tag-push `v0.2.56-dev` pending Klas-GO → dev-deploy → deploy-verifiering (explain-search-mode EXPLAIN-bekräftelse + perf-mål). Frontend (sök-skeleton) på Vercel vid main-push.
