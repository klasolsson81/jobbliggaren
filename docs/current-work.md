...
**Status:** **TD-13 FAS 3.5 (PII-FÄLT-KRYPTERING KMS-ENVELOPE) — C1/C2/C3 + HOTFIX LEVERERADE & PUSHADE, MAIN GRÖN PÅ `bbf8081` (origin/main synkad). C4 ARCHITECT-DESIGN LÅST, EJ PÅBÖRJAD. Session-checkpoint 2026-05-18 (§1.5 context-budget; kvalitet>tempo, ej Klas-besluts-stopp).**
...
**NÄSTA SESSION — återuppta C4.2 (mekanik LÅST #1c, Microsoft Learn-verifierad; läs `docs/reviews/2026-05-19-td13-c4-gate-and-mechanic.md` HELA + ADR 0049 Mekanik-not 6 + `docs/reviews/2026-05-18-td13-*`):** `resume_versions.content` (JSONB→text). **C4.0 KLAR — gate-utfall RÖD bekräftat** (`ValueConverter.ConvertFromProvider` kör FÖRE `IMaterializationInterceptor.InitializedInstance`); probe → permanent invariant-regressionsvakt `ResumeContentMaterializationProbeTests` (1 [Fact] grön). **C4.1 KLAR** — additiv migration `20260519060041_AddResumeVersionContentEnc` (`content_enc text NULL`, icke-destruktiv, snapshot oförändrad medvetet). **C4.2 mekanik LÅST #1c** (ADR Mekanik-not 6): `builder.Ignore(rv => rv.Content)` + string-shadow `ContentEnc`→`content_enc`; interceptor-paret äger ResumeContent↔JSON↔ciphertext på shadow-strängen (write: serialize→encrypt→`entry.Property("ContentEnc").CurrentValue`; read: shadow→decrypt→FromJson→Content via reflection); `EncryptedFieldRegistry` Form B-map (`JsonSerializedVoField`); ResumeVersion ägar-resolution skugg-FK `ResumeId`→Resume.JobSeekerId (write) / ICurrentDataOwner (read); ValueComparer-frågan upphör (Content ej EF-tracked). **GO-villkor:** (1) **C4.2a mini-gate** (paritet C4.0) — empiriskt verifiera shadow-läsning i `InitializedInstance` under `AsNoTracking` FÖRST, blockerar Form B-read; (2) C4.2 mappar BÅDE legacy `content jsonb` + `content_enc` som shadows under backfill, read prefererar content_enc(sentinel) annars legacy, INGEN content-drop i C4.2; (3) Mekanik-not 6 STOPP V-flagg. **C4.3** markörer (CreateResume/UpdateMasterContent/GetResumeById — verifiera DeleteResume*-handlers) + arch-test (Content `Ignore`:ad om JsonVoField; ContentJsonOptions→delad SPOT). **C4.4** testsvit. Sedan **C5** BackfillFieldEncryptionJob (Hangfire-chassi + fitness `COUNT(*) WHERE content_enc IS NULL=0`; cutover-flipp = **Klas-STOPP** fitness-ratchet ADR 0045). **C6** crypto-erasure-hook i `AccountHardDeleter.HardDeleteAccountAsync` — anropa `IUserDataKeyStore.DeleteDataKeysAsync` INOM hard-delete-transaktionen (security C2/C3-överlämning, Art. 17-atomicitet) + CryptoErasure/SecurityLogging-tester. **STOPP I**-rapport (full svit + SQL-bevis taxonomi-sök intakt). **STOPP V** = Klas-GO tag-deploy v0.2.x-dev + rök-verify + raw_payload-generated-cols/taxonomi-regress + TD-13→tech-debt-archive (§9.7) + FAS 4-startprompt. **Senare (Klas-STOPP):** Beslut 5 Steg D drop `content` jsonb (destruktiv, separat commit, prod-verifiering).
