...
**Status:** **F6 P4 SÖK-INFRASTRUKTUR-FIX DEPLOYAD 2026-05-21 (HEAD `39cf768`, tag `v0.2.54-dev`, origin/main — live på dev).** Två problem fixade per §9.6: **P1 q-search-perf** (Approach A GIN trigram-index, CTO-dom 2026-05-20) + **P2 filter-bugg-ROT-ORSAK** (JobTechHit-POCO saknade klassifikations-properties → raw_payload utan occupation/workplace_address → generated columns NULL → ssyk/region-filter 0 träffar). **Deploy krävde 4 cykler** (v0.2.51 pg_trgm-permission → v0.2.52 ensure-extensions-CLI-mode + GIN-timeout → v0.2.53 CommandTimeout 600s → v0.2.54 partial-index-predikat-fix). **VERIFIERAT post-deploy:** P1 q-search 40s→1.6s cold/0.19s warm för specifika termer; P2 ssyk/region-filter fungerar (ssyk=fg7B_yov_smw→2, region=CifL_Rzy_Mku→12); recent-searches 57s→6.3s. **KVARSTÅR:** (a) P1 vanliga korta svenska termer fortf. långsamma (lärare 18.7s — trigram-svaghet, kräver ny CTO-rond: acceptera/FTS-hybrid/title-only); (b) P2 backfill av ~51k legacy-rader pending nästa `sync-platsbanken-snapshot` 02:00 UTC (admin-trigger avvecklad, TD-83) — idempotent UPSERT re-importerar med ny POCO. **Reviews:** code-reviewer 0/0/0 + security-auditor 0/0/0/0/0. **Tester:** Domain 399 + App.Unit 532 + Arch 70 + Integ 19 — gröna. **Commits:** 4 original (815e422/8417ec9/0c0c262/ad2bfc9) + 3 deploy-fix (e30e387/5bcae2c/39cf768). ADR 0061 Accepted. F6 P4b SavedJobAds avblockerad. Session-logg `docs/sessions/2026-05-20-2340-f6-p4-sok-infrastruktur-fix.md`.
