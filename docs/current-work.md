...
**Status:** **F2 JOBB-INGESTION ROTORSAK FIXAD (Commit 1+2 pushed 2026-05-16, HEAD `70a7c54`) — snapshot-jobbet 60 starts / 0 completes på dev (CloudWatch 4 dygn) pga uncaught Npgsql 23505 på `ix_job_ads_external_source_external_id`. Rotorsak: hela ~47k-snapshot-loopen i EN DI-scope → ackumulerad EF change-tracker + UnitOfWorkBehavior-SaveChanges bröt ADR 0032 §5 per-command-isolering vid dubbletter (snapshot ⊇ stream-infogat). Korpus fastnade ~5k (enbart stream-deltan) av ~47k. Fix: child-scope per item (senior-cto-advisor Variant B) + IAsyncEnumerable-streaming (~300MB OOM-defekt, del a) + rate-limiter bounded queue (del b). 933 tester gröna, build 0/0, code-reviewer 0 Blockers/Majors, dotnet-architect-formad. EJ live-verifierat (kräver deploy = Klas-GO). PENDING KLAS: (1) ADR 0032 §5-clarification + §9-amendment-förslag (visat i chatt — Förbud kräver GO), (2) Commit 3 (admin-async + `IBackgroundJobLauncher` + `SyncPlatsbankenSnapshotWorker` DisableConcurrentExecution), (3) initial-backfill `RecurringJob.TriggerJob` efter deploy, (4) cadence-beslut (CTO rek: behåll */10 + 0 2 oförändrade).**
