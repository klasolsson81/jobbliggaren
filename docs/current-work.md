...
**Pending för nästa fas (Klas-valt nästa spår; 5 punkter dragna 2026-05-23, sekvens ej låst):**

1. **Snapshot-retention** — rensa utgångna jobb i `sync-platsbanken-snapshot` (sannolik kandidat: löser korpus-drift och en del av recall-gap-observationen indirekt — utgångna jobb i index sänker recall mot färska Platsbanken-resultat)
2. **Landing live-stats** — ersätt hårdkodad `getLandingStats()` per ADR 0056-utbytespunkt
3. **Jobbkort-Spara/Har-ansökt** — F6 P4b SavedJobAds backend-prompt (paus-rivs vid återupptag av sök-spåret)
4. **Översiktssida `/oversikt`** — handoff-källa: `docs/jobbpilot-v3-bundle/JobbPilot/handoff-oversikt/`
