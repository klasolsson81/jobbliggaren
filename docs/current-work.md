...
**Status:** **FAS G1 (GRÖN ACCENT-IDENTITET + F4-BANNER, ADR 0068) BYGGD + reviews klara 2026-06-10 (branch `feat/design-g1-gron-accent-f4-banner`, PR mot main, bas-HEAD `2922a25`). KLAS-GO PÅ RENDERAD UI KVARSTÅR.** E1+E2a MERGADE (#39 `86b61ae` / #40 `f860ddf` / #41 `2922a25`). Klas levererade ny design-handoff (`docs/handoff-banner/`, Claude Design "F4 Hybrid") + chat-direktiv: bannern tillbaka (för tomt utan), accentbyte blå/navy→mörkgrön i HELA appen + landing, alla spec-filer uppdateras (CC-mandat). **G1 = app-wide accent-migration** (`--jp-accent-*`-ramp #15603F light/#6EE7A8 dark, alias-flip → Tailwind/shadcn gratis, navy = logo-only) **+ F4-banner på /jobb** (inramad gradient-platta 118° granskogsgrön, asymmetrisk, neutrala vita kontroller + ink-Sök, INGEN placeholder — Klas hård regel) **+ pagehero/empty-brand/landing-hero gradient-färg-swap + fokus-regelverk (grön/vit-i-gradient) + spec-sync** (ADR 0068 ny; CLAUDE.md §5.2 gradient-undantag; DESIGN.md §1.2+§3; principles/tokens/components/a11y-skills → grön kanon — **absorberar pending docs-drift**). CTO-domar: neutraler A (v3 består — referensens slate = stale scaffolding), scope i–iii, ny ADR (ej amendment), fas-namn G1, E2b–E2e pausade. design-reviewer 0 VETO/2 Major (åtgärdade in-block: CLAUDE.md-scope-rad + landing-CTA→ghost), code-reviewer 0 Block/1 Major (åtgärdad: components-skill-hover) + minors in-block; matchchip-mid→neutral GODKÄND; TD-108 lyft (pre-existing kontrast-borderlines, G1 låser status/neutraler). security-auditor EJ invokerad (motiverat: ren CSS/copy/spec, ingen PII/auth/secrets-yta per §9.2). tsc/eslint/build gröna, 721 vitest gröna. **KVARSTÅR: (1) Klas-GO på renderad UI (Vercel-preview — hans egen design, light+dark); (2) logo-översyn separat (guld #FFCD00 vs #E8C77B + og/twitter-wordmark).** Efter G1-merge: E2b–E2e återupptas (kommun-kaskad → facet-count/NBomber → chips efter semantik-GO → sortering).
...
| (denna) | — | feat/design-g1-gron-accent-f4-banner — ADR 0068 grön identitet + F4-banner (pending Klas-GO) |

---

## Pending operativt för Klas

1. **KLAS-GO PÅ RENDERAD UI — G1 grön identitet + F4-banner:** din egen design (handoff F4 Hybrid) implementerad app-wide. Granska Vercel-preview (G1-PR) light + dark: /jobb-bannern (platta, asymmetri, neutrala kontroller, ingen placeholder), accent-grönt överallt (nav/länkar/knappar/selektion/fokus), pagehero/landing-gradient, logo-kompassen FORTSATT navy+guld. Ge GO → CC sätter automerge. OBS: rendered avviker MEDVETET från handoff-PNG:erna i neutral-detaljer (v3-neutraler består — referensens slate var stale; CTO Beslut 1).
