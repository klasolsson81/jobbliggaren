...
**And the residual is where the PROJECTED breach lives, not an edge case beside it — the word is exact.** No app container on this box has yet lived 30 days, so nothing has *exceeded* the window; what was measured is the **absence of a bound**. `web` and `caddy`, the two whose bound is missing outright, had no rotated segment at all, so `jobbliggaren-logprune` cannot touch either of them yet. The mechanism this amendment lands does not reach today's worst case; it reaches every case that rotates at least once, which is not the same claim.
