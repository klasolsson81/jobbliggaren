...
- **Mekanism:** CI-jobbet `coverage` (`build.yml`) parsar `Summary.json` (`jq`, förinstallerat — ingen ny supply-chain-yta) per assembly (`.coverage.assemblies[].name/.coverage/.branchcoverage`) och fail:ar steget under något golv. **Enforce:ad** (denna Accepted-flip): `continue-on-error: true` + `exit 0` borttagna; `coverage` ligger i `ci.needs` + result-check (uppräkningen av `ci.needs` har ETT hem — `build.yml`; en lista över trädet går stale medan en regel inte gör det). Det snabba `backend`-jobbet rörs ej (fast fail-feedback bevaras); coverage kör parallellt. *(Historik: gaten levererades PROPOSED — `continue-on-error` + `exit 0`, ej i `ci.needs` — i commit `2d262ee` och aktiverades vid denna Accepted-flip.)*

> **Amendment 2026-08-02 (#1145):** klausulens formulering *"Det snabba `backend`-jobbet rörs ej (fast fail-feedback bevaras)"* beskrev ett förhållande som upphört, och den är ursprungshemmet för det påstående `build.yml` citerade. Mätt 2026-08-02 över de tio senaste `main`-körningarna (daterade 2026-07-17..07-25 — ingen `main`-körning har registrerats sedan dess trots 53 landade commits, GITHUB_TOKEN-suppressionen per CLAUDE.md §6.5): `backend` 12m15s–14m42s (de **sex** senaste 14m08s–14m42s), `coverage` 13m05s–16m11s med median 14m38s. **PARAT på samma tio körningar är fail-feedback-deltat median +43s, spann −1m37s … +2m26s — en av tio över två minuter.** (Ett tidigare utkast skrev "under två minuter" genom att ställa det ena jobbets SPANN mot det andras MEDIAN; det är två populationer och påståendet blir falskt på den observationen. Medianen bär slutsatsen, det universella påståendet gör det inte.) — `backend` är inte längre "det snabba jobbet", och sedan #1145 bär båda jobben samma `timeout-minutes: 25`. **Beslut 1–5 är OFÖRÄNDRADE:** coverage-gaten, per-lager-golven och ratchet-vägen är orörda; enbart premissen i klausulens motivering har ändrats. **Det gällande skälet till att `coverage` är ett EGET jobb är parallellism** — två sekventiella svit-körningar ≈ 30 min väggklocka — **inte fail-feedback-hastighet.**

> **Amendment 2026-09-25 (#1785):** `backend`-jobbet är **pensionerat**; `coverage` är svitens **enda** körning i `build.yml` och det enda .NET-testjobbet i `ci.needs`. Grunden är #1785:s CI-audit: de två jobben körde identiska 24 541 tester och skilde sig bara i instrumenteringen. **Parat på samma huvuden i PR #1839:s egna `pull_request`-körningar** (2026-09-25; `gh run view <id> --json jobs` regenererar): `da8b3ce4` backend 11,5 / coverage 11,6 min · `b24c6871` 10,7 / 7,7 · `6cd8c902` 8,3 / 10,1 — varken jobbet eller dess frånvaro flyttar `ci`:s väggklocka (10,3–12,2 min på samma körningar), och `backend` bar ingen grind `coverage` saknar: ett fallerande test fäller `scripts/coverage.sh` (`set -euo pipefail`) och därmed jobbet. Vad som tas bort är ~10 runner-minuter per push och en andra 25-minuters-timeout-yta (den som #1785 filades på). **Beslut 1–5 är OFÖRÄNDRADE**; den 2026-08-02 amenderade Mekanism-klausulen består som historik, men dess kvarvarande skäl för ett *eget* coverage-jobb (parallellism mot `backend`) är tomt sedan det andra jobbet inte finns. Fail-feedback-hastigheten ägs nu av sviten själv: samma audit tog `Api.IntegrationTests` från 18m39s till 6m20s på runnern (PR #1839, `OwnContainerPerTestTests` pinnar återfallet).
