...
> **Amendment 2026-09-25 (#1785):** `backend`-jobbet är **pensionerat**; `coverage` är svitens **enda** körning i `build.yml` och det enda .NET-testjobbet i `ci.needs`. Grunden är #1785:s CI-audit: de två jobben körde identiska tester och skilde sig bara i instrumenteringen. **Parat på samma huvuden i PR #1839:s egna `pull_request`-körningar** (2026-09-25, runs 36096772636 / 36098876003 / 36100193300; `gh run view <id> --json jobs` regenererar): `da8b3ce4` backend 11,5 / coverage 11,6 min · `b24c6871` 10,7 / 7,7 · `6cd8c902` 8,3 / 10,1 — och `backend` bar ingen grind `coverage` saknar: ett fallerande test fäller `scripts/coverage.sh` (`set -euo pipefail`) och därmed jobbet. Vad som tas bort är ~10 runner-minuter per push och en andra 25-minuters-timeout-yta. **Beslut 1–5 är OFÖRÄNDRADE**; den 2026-08-02 amenderade Mekanism-klausulen består som historik, men dess kvarvarande skäl för ett *eget* coverage-jobb (parallellism mot `backend`) är tomt sedan det andra jobbet inte finns. Fail-feedback-hastigheten ägs nu av sviten själv: samma audit kortade `Api.IntegrationTests` på runnern (PR #1839 bär mätningen; `OwnContainerPerTestTests` pinnar återfallet).
