...
> **Amendment 2026-07-27 (#836):** raden ovan räknade tidigare upp `ci.needs`, och den uppräkningen blev falsk om trädet. `ci.needs` fick jobbet `scripts`, som kör fixturerna för `.github/scripts/is-pure-base-merge.sh` (merge-grindens predikat). *(Ögonblicksbild per 2026-07-27, i ett daterat amendment som får bära en: `[backend, frontend, coverage, scripts]`. Den gällande uppräkningen har ETT hem — `build.yml`.)* **Beslut 5 och Beslut 6 är OFÖRÄNDRADE** — `scripts` är varken en perf- eller audit-mekanism, var aldrig observe-only, och har ingen distributionsstabilitet att invänta; det är en deterministisk fixtursvit och blockerande från början. Ingen observe-only-mekanism har flippats, så Beslut 6:s ratchet är orörd, och coverage-gaten är oförsvagad. Uppräkningen är ersatt av regeln för att en lista över trädet går stale medan en regel inte gör det.
