...
| **N-1** | **Access-loggning för token-bärande e-postlänk-rutter** (`/bekrafta-epost`, `/bekrafta-konto`, `/aterstall-losenord`): EU-residens + query-string-scrubbing + definierad retention, inkl. Referer-ledet — normativ spec i `Amendment 2026-08-11` | **Minor** (ärvd: security-auditor 2026-07-06, #679 FE-granskningen, eskalerad till Klas) — **bekräftad oförändrad av security-auditor 2026-08-11** vid granskningen av PR #1313: flip-villkoret mättes då som icke-uppfyllt på residensbenet, båda hoppen är EU (Netcup Nürnberg per `Amendment 2026-08-04` §1; OVH `eu-west-par` per `vps-deploy-stack.md` rad 27c, mätt 2026-08-09) — **blir Blocker om:** *"prod access-logging for this route captures AND retains the query-string in a non-EU or over-retention sink"* (#706, verbatim). **Omgraderas obligatoriskt vid den andra security-auditor-granskningen före första beta-data** (samma klausul som M-5b bär): retentionbenet är i dag **odefinierat**, inte enbart osatt, och en odefinierad gräns är ett Art. 5(1)(e)-fel i sig — inte en osatt siffra | [#706](https://github.com/klasolsson81/jobbliggaren/issues/706) — **kvarstår ÖPPEN tills en accesslogg som uppfyller specen finns** (spec levererad = schemaläggning, inte stängbart faktum) |
...
**Token-bärande rader KAN alltså nå både den åldersobundna lokala driver-loggen och
off-box-arkivet redan i dag — vid 5xx på en token-bärande request — inte först när ett
`log`-direktiv landar.** Om lådans logg redan bär riktiga sådana rader är eskalerat till
...
- **G3 — definierad retention:** en beslutad tidsgräns för accessloggen. Siffran är Klas
  ...
  båda dagens lager (`json-file`, OVH `hostlogs/`) saknar åldersgräns, och `http.log.error`
  skriver redan i dem (se Nuläge).
