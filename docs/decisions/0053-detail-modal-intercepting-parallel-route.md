...
> **⚠ AMENDAD 2026-05-19 + 2026-06-19 — se [Amendment 2026-05-19 — Match-presentation Fas-4-gated](#amendment-2026-05-19--match-presentation-fas-4-gated) och [Amendment 2026-06-19 — Match presentation redesigned: category-primary match replaces the percentage](#amendment-2026-06-19--match-presentation-redesigned-category-primary-match-replaces-the-percentage) nedan. Originaltexten nedan bevaras oförändrad; amendment-lagren gäller dels Fas-tillämpningen (2026-05-19: presentation gäller från Fas 4, Fas 3-modalen renderar ingen match-sektion), dels presentationsformen (2026-06-19: mono `"92% match"` ersätts av category-primary match — verdict + matched/missing per dimension; procent-cirkel-förbudet kvarstår oförändrat). Amendment 2026-05-23 rör inte match-presentation (se dess "Vad amendment INTE rör").**
...
## Amendment 2026-06-19 — Match presentation redesigned: category-primary match replaces the percentage

> **Amendment 2026-06-19 (Klas product pivot 2026-06-19, AskUserQuestion GO; senior-cto-advisor mechanism rulings — ADR 0076 Accepted 2026-06-19):** The Fas-4-gated deferral imposed by Amendment 2026-05-19 above is lifted. F4-16 ("ADR 0053 Beslut 5 redesign — match presentation in job modal", ADR 0076's five-STEG sequence F4-12..F4-16) is the STEG that realises match presentation in the job modal. The presentation FORM specified by the original Beslut 5 above — mono `"92% match"` plus a 3-nivå-förklaring — is retired and replaced by **category-primary match**: a verdict plus matched/missing per dimension (ADR 0076, F4-16 gate: "Modal shows category-primary match (verdict + matched/missing per dimension); never '92% match'; never a percentage ring."). The original Beslut 5's ban on the percentage ring is **reaffirmed, not overturned** — this amendment retires the *number*; the *ring* was already forbidden and stays forbidden.

> **Why the number goes:** the Goodhart guard. ADR 0076 Alt B ("Expose a 0–100 numeric match score in the DTO") was considered and rejected. Its **Against**: "Violates the Goodhart guard (ADR 0071) — an opaque aggregate score becomes the target rather than genuine qualification signal. Users optimise CVs for the score, not for the job. Explicitly named as a forbidden pattern in CLAUDE.md §5 (deterministic engines block) and ADR 0071. ADR 0053 original Beslut 5 also forbids the percentage ring." Its **Verdict**: "Rejected. The Goodhart guard is non-negotiable." ADR 0076 Decision 4 makes the guard structural end-to-end: the sort-key computed for "sort by match" lives ONLY in the Infrastructure sort path — never returned in any DTO, never persisted, never rendered to the user — pinned by an architecture test asserting no numeric total field exists on `MatchScore`/`FullMatchScore`. The user always sees WHY: matched/missing per dimension, never an opaque aggregate (ADR 0076 Decision 4).

> ADR 0053 remains **Accepted**. This is an amendment layer, not a supersession — the original Beslut 5 text above is preserved unchanged (ADR immutability, Nygard 2011), as both amendments above already state for their own scope.

> **Forward-note — this amendment records 2026-06-19 and nothing later.** ADR 0076 has amended the modal's match section again since, and both times recorded the change in its own file rather than delegating it here: Amendment 2026-06-20 §6 ("Cross-reference updates") makes the modal's must-have row grade-load-bearing and adds a must-have reason line to the modal copy — "Structural/DTO shape unchanged; copy + grade-reason line only" — and Amendment 2026-06-27 revisits the same row and concludes "**ADR 0053 modal copy:** no structural change. … No new copy variant needed." Neither is dangling, because neither claims a home in this file. A reader of Beslut 5 should therefore treat ADR 0076's amendment history, not this section, as the live specification of the modal's match section.
