...
**Amendment 2026-07-28:** Vuln-grinden får acceptera bara det den inte kan reparera; en override-nyckel grindas bara för att partitionera (#1130) — se [§Amendment 2026-07-28](#amendment-2026-07-28--the-blocking-vuln-gate-may-accept-a-risk-only-where-repair-is-unavailable-1130).
...
## Amendment 2026-07-28 — the blocking vuln gate may accept a risk only where repair is unavailable (#1130)
...
ignored: `pnpm.overrides`, `pnpm.auditConfig`."* The summary line loses its
...
- **What holds the line is the CI pin, not the mechanism.** All five `pnpm/action-setup@v6`
  call sites pin `version: 9`, so CI is unexposed today. **Raising that pin past 9 is a
...
The new home is `pnpm-workspace.yaml` (pnpm's own settings documentation). Moving both
keys there is deliberately **not** done here: it is a change to how every pnpm invocation
