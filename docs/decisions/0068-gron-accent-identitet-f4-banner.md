...
### Implementerings-notat 2026-07-03 (#549 WS1) — E2f amended: headings move to the navy ramp

**Source:** Klas directive, issue #549 (2026-07-03): "dark-green or dark-blue bold headings are explicitly approved … colours are welcome where they improve clarity." Decision-maker: senior-cto-advisor, D1 (`docs/reviews/2026-07-03-549-typo-doctrine-cto.md`). Additive amendment to the 2026-06-11 E2f note above; E2f's Beslut-1 mechanics and its interaction-colour core are not reopened — only the "titlar/rubriker = ink" letter of E2f is relaxed.

- **Headings move from ink to the navy ramp.** New role tokens `--jp-heading-1` (page-title tier, `--jp-navy-700` `#133F73`, 10.57:1 on white) and `--jp-heading-2` (section/overlay tier, `--jp-navy-800` `#0A2647`, 15.20:1 on white) carry the top two heading tiers; h3/h4 stay ink-1 bold, unchanged. This relaxes E2f's *letter*, not its *core*: E2f moved "titlar" (and träffräknar-tal, date-numerals) off accent-700 onto ink-1. The rule now reads **"rubriker = navy (an information colour with no interactive role), tal = ink"** — headings get their own hue; numerals (match counts, dates, IDs) stay exactly where E2f put them, on `--jp-ink-1`.
- **E2f's core stands unchanged.** "Grönt = interaktion, inte information" is untouched — `accent-700` remains the sole interaction hue (links, focus, selection/active row, control states, semantic badges/pills, the notice category system). "Tal = ink" stands — hit counters, date numerals, and similar figures stay `--jp-ink-1`, never navy, never accent. Only the third leg of E2f ("titlar = ink") is amended; the other two are load-bearing and untouched.
- **Affordance rationale (why navy, not another hue).** Navy carries no interactive meaning anywhere in the app — links, focus rings, selection, and control states are all green (`accent-700`, Beslut 1 above). Routing headings to navy keeps *information* (headings) and *interaction* (links/controls) on strictly separate hues, so a navy heading is never mistakable for a clickable affordance — the same separation this ADR's E2f note already protected, now honoured with a distinct information hue instead of a monochrome one. Precedent: GOV.UK's dark blue-black headings paired with a distinct link colour — the civic register CLAUDE.md §1 targets.
- **Navy-ramp F-cleanup plan REVOKED.** The `globals.css` `:root` comment on the navy ramp ("Var LOGO-ONLY … navy-rampen är nu helt utan konsument och städas i egen F-städ-fas (ADR 0068-amendment)") is superseded by this note: the ramp is **not** cleaned up. It is re-legitimised as the heading-colour role bearer (`--jp-heading-1`/`--jp-heading-2` resolve to `--jp-navy-700`/`--jp-navy-800`) alongside its pre-existing logo-substrate role (`.jp-brand` currentColor). The comment is updated in the same PR (WS1) to record the revocation.
...
- **Status: proposed amendment, pending Klas GO.** This amendment rides CLAUDE.md §12's STOPP class (E2f override, coloured headings) — implemented on branch `refactor/typo-ws1-doctrine` (commit `8711c467`), **never automerge**. It stands as a proposed amendment to E2f until Klas GOes the WS1 merge; design-reviewer rendered-veto remains open on the exact rung (navy-700 vs navy-800), the coloured-level split, and the auth-h1 20→32 jump (D1's "Bearer & gates"). See also the accompanying ADR 0052 amendment note (Beslut 5, `--text-h1` 28→32 re-alignment) landing in the same PR.
