...
- **It does not make anyone aware of a competent root attacker.** Root can `auditctl -e 0`, erase
  ...
  trail — not an alarm.**
- **It does not lift ADR 0123's scope limit.** ⚠ **And the exit does not rest on this capability
  arriving.** Both mitigations the acceptance named are void as written — measured 2026-08-17, the
  derivation in `vps-base-hardening.md` §11 — so the exit at the pre-real-data boundary is
  **re-grant**, not **close**. Read it there; this file restates none of it.
