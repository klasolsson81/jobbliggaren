...
| `jbl-deploy` | the operator's own `git pull`, when it brings a compose change; an operator editing `.env` by hand | **not the reconcile timer** — reconcile never pulls, so this family is silent on the hourly cadence and fires only on an attended visit. Expect it during a deploy visit (`registration-gate.md` and `master-key-ops.md` both prescribe one) and treat an unattributable firing as a finding |
