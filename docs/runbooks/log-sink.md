...
| **The prune timer is on P3's floor** | `grep FLOOR_TIMERS /opt/jobbliggaren/deploy/systemd/jobbliggaren-heartbeat.sh` on the box, then `journalctl -u jobbliggaren-heartbeat` after a hand run, read for **`heartbeat: all predicates hold`** | **Both halves measured 2026-09-03.** *Repo:* the sixth name landed after the arming and never before it — `check_floor_timers` measures `is-enabled` **and** `is-active`, so the reverse order fails the heartbeat on the next pull. *Box:* the clone pulled that commit the same day (`da2d18f`), `grep` there returns the six-name constant, and a hand run reads **`heartbeat: all predicates hold`** at 18:56:59 box time. **That line is what separates "P3 now names the timer" from "P3 now fails on it"** — the floor is satisfied, not merely enlarged. ⚠ **Neither the unit's exit status nor an empty `systemctl --failed` may stand in for it** — the script declares it always exits 0, and a disabled timer never becomes a failed unit, so both are blind to P3 by construction. The window between merge and pull ran the OLD five-name constant, which is the safe direction and cannot page; it is the same window the logship pair carried on 2026-08-18 | 2026-09-03 |
...
| **The `json-file` residual reaches a real age on a real container** | oldest line in the **live** segment per container (`head -n1`, JSON `time` field) against the 30-day window; rotated-segment count in the same root shell | **`jobbliggaren-redis` carries a live segment whose oldest line is 2026-08-20 — 330 KB, never rotated — so on present behaviour it reaches 30 days on 2026-09-19.** `seq` is second at 2026-08-23. Both sit below `max-size`, so neither will rotate its way under a bound; `api`, `web` and `caddy` read 2026-09-02 only because the deploy replaced those containers. ⚠ **That a restart resets the clock is the defect, not an exception to it** (ADR 0024 Amendment 2026-08-28 §3): the age of retained data equals the container's own lifetime, and `redis` is simply the first container to demonstrate it with a real number. ⚠ **This cell records the AGE. The personal-data question it used to defer is now SETTLED, and the answer moves the residual rather than removing it.** `security-auditor` 2026-09-03, configuration measurement plus a log census: **`redis` carries none** — the configuration surface is empty (no `--loglevel`, no `--logfile`, no mounted conf), and redis has no server-side command log to the logfile at any level, since `MONITOR` streams to a client, `SLOWLOG` is in-memory and AOF is a datafile in the volume. **`seq` carries none** — 6 of 45 015 lines matched her refusal-vocabulary pattern (the six alternates are in her report), all benign (one startup banner, five retention lines matching a millisecond figure), with the ingestion gate measured armed since 2026-08-23; 44 900 of the 45 015 were shape-classified, 115 left unenumerated. **`postgres` carried them** — and it is the only one of the three that rotates, so the prune reaches it. ⛔ **So the dated crossings above are not Art. 5(1)(e) deadlines**, and the residual moved to a container with **no** dated crossing at all, which is worse for detection rather than better | 2026-09-03 |
| **`log_error_verbosity=terse` keeps constraint KEY VALUES out of the postgres container log, on the send path the app actually uses** | two throwaway `postgres:18.3` containers — the box's own image — one with the flag and one without, as a control in both directions; provoke a unique violation, then grep the CONTAINER LOG (never the client output) for the offending value. ⚠ **Send it with BOUND PARAMETERS** (`psql`'s `\bind`, the extended protocol Npgsql uses) — an inline literal measures a path the app does not take | **Measured 2026-09-03, and the control crosses the threshold.** *Without the flag:* one `ERROR: duplicate key value…`, one `DETAIL:` line, and the pnr-shaped value present **twice** in the container log. *With it:* the same ERROR, **zero** `DETAIL:` lines, `STATEMENT: INSERT INTO t VALUES ($1,$2)` — placeholders only — and the value present **zero** times. ⚠ **An inline-literal send leaks anyway, through STATEMENT rather than DETAIL**, and that arm was measured too: the value survives once. It is not the app's path (EF/Npgsql binds), but it is why this row names the protocol rather than only the flag. The client still receives its DETAIL, so app-side diagnosis is unchanged; what is given up is DETAIL, HINT and CONTEXT **in the server log**, on every error — which is the thing being removed on purpose. `log_parameter_max_length_on_error` already defaults to 0, so STATEMENT never carried bound values | 2026-09-03 |
