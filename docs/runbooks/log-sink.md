...
| **The LIVE segment survives a real (non-dry) run** | §6 step 6, immediately after §6 step 5 | **Measured at the install 2026-09-03, and as an EFFECT rather than a presence check.** All eight existing containers reported `live=present` after the first `dry_run=0` run, and the stronger form was taken in the same visit: every live segment's **inode was unchanged** across the run — `api` 2991163, `worker` 2991165, `web` 2991176, `caddy` 2991130, `postgres` 520207, `redis` 2991191, `seq` 2991142, `migrate` 2991150 — so the file was not deleted and recreated, it is the same file. **A presence check alone could not have told those apart.** Three of them grew across the run (`worker`, `postgres`, `seq`), which is what shows the containers kept writing through it. The coupling this row exists to protect was measured beside it: `docker logs --timestamps --since` exits **0** for all eight and returns their real line counts, and that is the input `jobbliggaren-logship.sh` dies on. `jobbliggaren-logprune.test.sh` asserts the absence in every fixture case that prunes, but a fixture is not this box | 2026-09-03 |
