...
| **The prune resolves the real container layout, and removes nothing it may not** | pipe `jobbliggaren-logprune.sh --dry-run` to the box over stdin (`ssh … 'sudo bash -s -- --dry-run' < …`) so no file is created and nothing is deleted; read the per-container lines | **Measured 2026-08-28, and it is the half the fixtures cannot reach.** All four app-stream containers resolved, each directory found, and the run reported `pruned=0 kept=0 unreadable=0` — because **none of them owned a rotated segment at 20:54 UTC that day** — a reading, not a standing state: `worker` was then at 7.4 MB of its 10 MB cap and rotates within the hour, after which this cell's premise is false and the run would report a non-zero `kept`. That is the correct answer on this box and it is also the finding: the mechanism is inert here today, which is precisely the residual §6 names and why [#1170](https://github.com/klasolsson81/jobbliggaren/issues/1170) does not close on it. The fixture suite proves the predicate; this proves the path shape and the `docker inspect` call against a real host. **It does not prove the glob** — a mistyped pattern matches nothing in exactly the same way, which is the fail-open shape row 4 above already records retiring once. The glob is pinned in the fixture suite, where a segment exists for it to match | 2026-08-28 |
| **The prune REMOVES an aged rotated segment, measured as an effect** | let a segment rotate, then list the directory after it passes the window | **Not measured, and not measurable yet on this box.** No app-stream container has rotated at all, so there is no segment to age out. This cell is scheduling, not a claim — it comes due at the first rotation plus the window, and a green unit run is not a substitute for it | |
...
**That residual is not academic — it is where the breach was measured.** On 2026-08-28 `web` had
...
# expect: a "logprune: window=30d cutoff=… dry_run=1" line, then one line per container,
#         then "logprune: pruned=N kept=N unreadable=N". Any "WOULD PRUNE" line names a file.
