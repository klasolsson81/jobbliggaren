...
if [ "$CODE" = 000 ]; then
  echo "GATE FAILED: no HTTP response from $SEQ_IP - the question was never asked."; exit 2
elif [ "$CODE" != 200 ]; then
  echo "GATE FAILED: GET answered $CODE - the question was never answered, which is NOT absence."; exit 2
fi
python3 -c 'import json,sys; d=json.load(open("/tmp/seq.retention")); print("GATE OK, policies:", [x.get("Id") for x in d]) if d else sys.exit("GATE FAILED: retention list is EMPTY - step 7 has not run. Do NOT continue.")'
)
echo "gate exit: $?"
# expect: GATE OK, policies: ['retentionpolicy-NN']   then   gate exit: 0
...
```bash
{ sudo grep -v -E '^SEQ_(INGEST_API_KEY|SERVER_URL|ADMIN_PASSWORD)=' /opt/jobbliggaren/deploy/.env
