...
load-bearing. An interactive operator uses the editor:
`sudo nano /opt/jobbliggaren/deploy/.env`. A session with no TTY uses the atomic form step 8
writes out, with one key here instead of three — written once there rather than improvised twice.
...
steps 4–7 and do not sign in again: a second sign-in **invalidates the first session** (§4).

```bash
CODE=$(curl -s -o /tmp/seq.retention -w '%{http_code}' -b /tmp/seq.jar "http://$SEQ_IP/api/retentionpolicies")
if [ "$CODE" != 200 ]; then
  echo "GATE FAILED: GET answered $CODE, not 200 - not signed in. A non-200 is NOT absence."
else
  python3 -c 'import json,sys; d=json.load(open("/tmp/seq.retention")); print("GATE OK, policies:", [x["Id"] for x in d]) if d else sys.exit("GATE FAILED: retention list is EMPTY - step 7 has not run. Do NOT continue.")'
fi
# expect: GATE OK, policies: ['retentionpolicy-NN']
# Anything else is STOP. The two failures are DIFFERENT and this block keeps them apart:
# an empty list means step 7 did not run; a 401 means you are not signed in and the
# question was never asked at all. Neither of them is "probably fine".
```
...
```bash
sudo sh -c 'cd /opt/jobbliggaren/deploy
T=$(mktemp .env.XXXXXX)
{ grep -v "^SEQ_" .env; printf "SEQ_INGEST_API_KEY=%s\nSEQ_SERVER_URL=http://seq:5341\nSEQ_ADMIN_PASSWORD=%s\n" "$1" "$2"; } > "$T"
chown root:root "$T"; chmod 600 "$T"; mv "$T" .env' _ "$TOKEN" "$NEW_PW"
sudo stat -c 'mode=%a owner=%U:%G' /opt/jobbliggaren/deploy/.env
# expect: mode=600 owner=root:root
```

**The `_ "$TOKEN" "$NEW_PW"` tail is load-bearing and its absence is silent.** Inside `sh -c`'s
single quotes a `$TOKEN` from the calling shell does not expand, so the same block without the
positional arguments writes `SEQ_INGEST_API_KEY=` and `SEQ_ADMIN_PASSWORD=` — **empty**, with no
error — and an empty ingest key is the one input step 5 measured answers 401 to every event.
...
| **§3 step 8's precondition gate tells "no policy" apart from "not signed in"** | run step 8's gate block verbatim against `datalust/seq:2026.1` in all three states | **Three states, three verdicts, image-side 2026-08-24.** *No policy:* authenticated GET `200` with `[]` -> gate stops with "step 7 has not run", exit 1. *Not signed in:* `401` -> gate stops on the code, exit 2, and never reads it as absence. *Policy present:* `200`, one id -> gate passes, exit 0. That the two failures stay distinguishable is the whole property; the row above records why an empty list is a real reading here. `POST` answers `201` with `DataSource Stream` and `RemovedSignalExpression null`, and `DELETE /api/retentionpolicies/<id>` answers `200` and returns the list to `[]` — which is how the empty state was reached a second time rather than assumed | 2026-08-24 (image) |
| **§3 step 8's non-interactive `.env` write is atomic and preserves `600 root:root`** | the step 8 block, run against a `600 root:root` `.env` in a throwaway `ubuntu:24.04`; the target's **inode** before and after, plus `stat` mode/owner | **Both arms measured 2026-08-24.** *Naive in-place redirect:* inode **unchanged** — the target is rewritten in place, so reconcile's hourly read can catch it half-written. *The sanctioned form:* inode **changed**, `mode=600 owner=root:root`, the non-`SEQ_` line preserved, no temp file left behind, idempotent on a second run (3 `SEQ_` lines, not 6), and the whole probe ran with **no TTY**. `mktemp` itself creates `600` owned by the caller, stated rather than assumed. ⚠ **Measured separately because it fails silently:** the same block without the `_ "$TOKEN" "$NEW_PW"` tail writes both secrets **empty**, since `sh -c`'s single quotes do not expand the caller's variables. Values round-trip byte-exact through `printf %s`, including `+ / = $ % @` and a space | 2026-08-24 (image) |
