...
| **The LIVE segment survives a real (non-dry) run** | §6 step 6 (`sudo test -f …/<id>-json.log` per container), and in the same visit `stat -c %i` on every live segment **before and after** the run, plus `docker logs --timestamps --since` per container read for its exit status. ⚠ **The last two are what the verdict below rests on, and §6 step 6 alone does not produce them** — it stats for presence and nothing more | **Measured at the install 2026-09-03.** All eight existing containers reported `live=present` after the first `dry_run=0` run; every live segment's inode was **identical before and after** — `api` 2991163, `worker` 2991165, `web` 2991176, `caddy` 2991130, `postgres` 520207, `redis` 2991191, `seq` 2991142, `migrate` 2991150 — three of them (`worker`, `postgres`, `seq`) grew across the run, and `docker logs --timestamps --since` exited **0** for all eight, which is the input `jobbliggaren-logship.sh` dies on. ⚠ **That run reported `pruned=0`, so this reading does not discriminate.** An unchanged inode is also exactly what a run with nothing to delete produces, so the evidence is consistent with the design sparing the live segment AND with the run simply having had no work — it does not separate them. **This cell comes due again at the first run that actually prunes**. `jobbliggaren-logprune.test.sh` asserts the absence in every fixture case that prunes, but a fixture is not this box | 2026-09-03 |
| **The prune timer is on P3's floor** | on the box, in this order and read as three separate facts: `sudo git -C /opt/jobbliggaren rev-parse --short HEAD` (checkout state — reconcile moves images and never git, so it must be read on its own), then `grep FLOOR_TIMERS deploy/systemd/jobbliggaren-heartbeat.sh` (file content), then `journalctl -u jobbliggaren-heartbeat` after a hand run, for **`heartbeat: all predicates hold`** | **Both halves measured 2026-09-03.** *Repo:* the sixth name landed after the arming and never before it — `check_floor_timers` measures `is-enabled` **and** `is-active`, so the reverse order fails the heartbeat on the next pull. *Box:* the clone was at `da2d18f` and the grep returned the six-name constant at 18:52; a hand run at 18:56:59 read `heartbeat: all predicates hold`. ⚠ **That journal line is an AGGREGATE and names no timer**, so a run against the old five-name constant writes an identical line — what makes the two readings compose is that the grep was taken after the pull and before the run, which is why the readings are timestamped rather than summarised. ⚠ **Neither the unit's exit status nor an empty `systemctl --failed` may stand in for it** — the script declares it always exits 0, and a disabled timer never becomes a failed unit. The window between merge and pull ran the OLD five-name constant, the safe direction: it cannot page. Same window the logship pair carried 2026-08-18 | 2026-09-03 |
...
| **The `json-file` residual reaches a real age on a real container** | oldest line in the **live** segment per container (`head -n1`, JSON `time` field) against the 30-day window; rotated-segment count in the same root shell | **`jobbliggaren-redis` carries a live segment whose oldest line is 2026-08-20 — 330 KB, never rotated — so on present behaviour it reaches 30 days on 2026-09-19.** `seq` is second at 2026-08-23. Both sit below `max-size`, so neither will rotate its way under a bound; `api`, `web` and `caddy` read 2026-09-02 only because the deploy replaced those containers. ⚠ **That a restart resets the clock is the defect, not an exception to it** (ADR 0024 Amendment 2026-08-28 §3): the age of retained data equals the container's own lifetime, and `redis` is simply the first container to demonstrate it with a real number. ⚠ **This cell records the AGE. The personal-data question it used to defer is now SETTLED, and the answer moves the residual rather than removing it.** `security-auditor` 2026-09-03, configuration measurement plus a log census: **`redis` carries none** — the configuration surface is empty (no `--loglevel`, no `--logfile`, no mounted conf), and redis has no server-side command log to the logfile at any level, since `MONITOR` streams to a client, `SLOWLOG` is in-memory and AOF is a datafile in the volume. **`seq` carries none** — 6 of 45 015 lines matched her refusal-vocabulary pattern (the six alternates are in her report), all benign (one startup banner, five retention lines matching a millisecond figure), with the ingestion gate measured armed since 2026-08-23; 44 900 of the 45 015 were shape-classified, 115 left unenumerated. **`postgres` carried them** — and it is the only one of the three that rotates, so the prune reaches it. ⛔ **So neither dated crossing above is an Art. 5(1)(e) deadline.** ⚠ **And the pnr half of the earlier reading was measured FALSE** (`senior-cto-advisor` 2026-09-03): the ingest job collides on `(source, external_id)`, and `(UserId, OrganizationNumber)` stores an HMAC token (ADR 0090 D5). The measured bearer was `normalized_user_name` — an email — via `UserNameIndex`, and it lives in ADR 0024 Amendment 2026-09-03 (2) §2. ⚠ **postgres is the one that is worse for detection**, because its crossing is not undated but RESET by every rotation, and it is the only one of the three that rotates | 2026-09-03 |
| **`log_error_verbosity=terse` keeps constraint KEY VALUES out of the postgres container log** | two throwaway `postgres:18.3` containers — the box's own image — one with the flag and one without, as a control in both directions; provoke a unique violation, then grep the CONTAINER LOG for the offending value. Three arms, all run: **bound parameters** (`psql`'s `\bind`, the extended protocol Npgsql uses), an **inline literal**, and the no-flag control | **Measured 2026-09-03, and the control crosses the threshold.** *Without the flag:* one `ERROR: duplicate key value…`, one `DETAIL:` line, offending value present **twice**. *With it, bound parameters:* the same ERROR, **zero** `DETAIL:` lines, `STATEMENT: INSERT INTO t VALUES ($1,$2)` — placeholders only — value present **zero** times. *With it, inline literal:* value survives **once**, through `STATEMENT` rather than `DETAIL`. That third arm is not the app's send path but is why this row names the protocol and not only the flag. ⚠ **Only the DETAIL arm was measured.** Postgres documents TERSE as excluding DETAIL, HINT, QUERY and CONTEXT from **every logged message**, not only errors; QUERY and CONTEXT were not exercised here and that wider cost is read off the documentation, not measured. ⚠ **Box half: OWED.** The flag reaches the box only at a `git pull` **and** an apply that RECREATES the container — a `command:` change, which reconcile's image pull does not deliver by itself. Until then the box runs the default. **And the flag bounds only FUTURE writes:** segments already in the box's postgres log keep their values until the recreate discards them or the prune reaches the rotated ones. ⓘ The value used in the fixture is pnr-shaped; that is the probe's own datum, not a claim about what production writes — ADR 0024 Amendment 2026-09-03 (2) §1 withdrew the pnr bearer | 2026-09-03 (image only) |
