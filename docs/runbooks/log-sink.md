...
| **The prune resolves the real container layout, and removes nothing it may not** | pipe `jobbliggaren-logprune.sh --dry-run` to the box over stdin (`ssh … 'sudo bash -s -- --dry-run' < …`) so no file is created and nothing is deleted; read the per-container lines | **Measured 2026-08-28, and it is the half the fixtures cannot reach.** All four app-stream containers resolved, each directory found, and the run reported `pruned=0 kept=0 unreadable=0` — because **none of the four owns a rotated segment**. That is the correct answer on this box and it is also the finding: the mechanism is inert here today, which is precisely the residual §6 names and why [#1170](https://github.com/klasolsson81/jobbliggaren/issues/1170) does not close on it. The fixture suite proves the predicate; this proves the path shape, the `docker inspect` call and the glob against a real host | 2026-08-28 |
...
for c in jobbliggaren-api jobbliggaren-worker jobbliggaren-web jobbliggaren-caddy; do
...
# expect: present, four times. A MISSING here is a stop-everything result: docker logs is the
...
alarm reporting the arming sequence rather than the box. Until step 7 lands, this timer is armed but
**on no alarm surface**, exactly as the logship pair was for three days; that gap is a known cost of
the ordering, not an oversight.
