...
| **The one-time setup completes with NO change to sshd** | the §3 command sequence, end to end | **Measured against `datalust/seq:2026.1` (`sha256:91e93ff2…`), not against this box:** login-with-`NewPassword` 200, gate PUT 200, `POST /api/apikeys` 201 with `['Ingest']`, `POST /api/retentionpolicies` 201. The box-side run is what this row still owes; the mechanism is no longer an assumption. ⛔ **THE DEBT IS NOW MEASURED RATHER THAN ASSUMED, AND IT IS THE WHOLE SEQUENCE:** read-only on the box 2026-08-23, **not one step of §3 has been run there**. `user-admin` carries `MustChangePassword: true`, so step 4's login was never completed; `apikey-default` carries `AssignedPermissions: []`, so step 5's ingest key does not exist; `RequireApiKeyForWritingEvents` appears nowhere in the metastore, so step 5's gate was never set; step 7's retention policy does not exist (the row below); and the box `.env` carries only `SEQ_ADMIN_PASSWORD`, so the provider was never attached. Regenerate the retention half with the row below; the rest reads the same store | 2026-08-11 (image only) · 2026-08-23 (box: none of §3 run) |
...
| **A retention policy exists on this box AT ALL — the question [#1170](https://github.com/klasolsson81/jobbliggaren/issues/1170) asks, and the one the row below silently presupposes** | the metastore's entity-id prefixes, with the other prefixes as controls: `sudo sh -c 'd=$(docker inspect jobbliggaren-seq -f "{{range .Mounts}}{{.Source}}{{end}}"); for p in retentionpolicy- role- user- signal- apikey-; do printf "%-18s %s\n" "$p" "$(cat "$d"/Documents/metastore.collection.*.docc | grep -ac "$p")"; done'`. **Read the controls and not only the answer** — they are the whole proof that the store is text-searchable, without which a zero measures the instrument rather than the box. `grep -c` counts LINES, not matches, which is harmless here only because the load-bearing figure is a zero | **NO POLICY IS SET.** `retentionpolicy-` returns 0 while every other prefix returns non-zero; the store's only `retention` string is a built-in role description. Corroborated three further ways the same day: the container's environment carries only `ACCEPT_EULA` and `SEQ_FIRSTRUN_ADMINPASSWORD`; `/data/Seq.json` has **no retention section**, and datalust document none — there is no `SEQ_RETENTION_*` and no config surface at all, so **this can never be repaired in compose, only by §3 step 7**; and `GET /api/retentionpolicies` answers 401, while a login on the `.env` password returns `{"Error":"A password change is required.","MustChangePassword":true}`. ⚠ **Nothing is currently AT RISK from this, and that is a separate measurement, not a softening of the one above:** `Seq__ServerUrl` is empty on api and worker, so the provider is not attached and the store holds zero application events. The exposure begins the moment step 8 sets it — which is why step 7 precedes it in §3 | 2026-08-23 |
