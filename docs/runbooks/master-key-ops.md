...
| `…/Email__Scaleway__SecretKey` | **the transactional-mail key, and the only secret here that DIES ON A DATE.** Scaleway caps an API key at one year and has no instance-role equivalent. **Current key: expires `2027-08-16`** (issued 2026-08-16, access key `…P9DRX`, bearer = the IAM *application* `Jobbliggaren`; read in the console 2026-08-16, #183 E4). Rotation is **#198**'s; the date is enforced by `EMAIL_SCALEWAY_KEY_EXPIRES_AT` in `deploy/.env`, which `--check` reads and trips on from T-30. **This row is a record, not the control** — a date that lives only in a document is a reminder with no reader, which is the defect that produced both |
