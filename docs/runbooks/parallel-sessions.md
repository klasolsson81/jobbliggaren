...
|---|---|---|
| **Matching** | `Domain/Matching/`, `Application/Matching/` (scorer, grading, profiles, notifications, jobs/BackgroundMatching+DigestDispatch, queries), `Infrastructure/Matching/`, `…/Configurations/UserJobAdMatch*` + `MatchPreferencesConverters`, `Api/Endpoints/MeJobAdMatchEndpoints`, `Worker/Hosting/BackgroundMatchingWorker`+`DigestDispatchWorker` | `app/(app)/matchningar/`, `components/matches/`, `components/job-ads/match-chip`, `lib/api/{job-ad-match,me-matches,match-count,skills}`, `lib/dto/{…,match-preferences}` |
