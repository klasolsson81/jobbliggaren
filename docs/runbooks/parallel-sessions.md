...
> Neither has ever done it.** The clause landed 2026-06-28 (`9e5582a5`, #324), in a
> commit whose own message records *"10 stragglers closed"* — that pass was run by
> that PR's owning session, the only actor ever observed closing stragglers here.
> Klas confirmed 2026-07-28 that he runs no such loop, and the babysitter prompt §8
> documents has never mentioned issues in any of its versions.
>
> It named an actor for exactly the case §8.1 had excluded itself from. §8.1 assigned
> close-out to the owning session on 2026-06-25 (`022f77ed`, #223), but under a
> heading gated on *no* babysitter running. The two therefore **partitioned** the
> space rather than contradicting each other — which is worse than a contradiction,
> because with a babysitter running neither text claimed the duty and nothing looked
> wrong. Removing that gate is what this change does. CLAUDE.md §6.5's next bullet
> and `session-start-template.md`'s expected-end-state joined on 2026-07-15
> (`e4a83649`, #900), together with a session-start audit that *reports* stale `wip`
> claims — but an audit that reports is not a holder that acts.
