...
the 2026-08-10 measurement covered **two of its five legs** — the apex MX and `_dmarc` — while the
...
| 36 | **CONTROL — the existing STRATO mail path is untouched by the SES work** | `nslookup -type=TXT strato-dkim-0002._domainkey.jobbliggaren.se 8.8.8.8` and `-0003` (both still `v=DKIM1`), `nslookup -type=MX jobbliggaren.se 8.8.8.8` (still `smtp.rzone.de`), `nslookup -type=TXT _dmarc.jobbliggaren.se 8.8.8.8` (**still `v=DMARC1;p=reject;` and still EXACTLY ONE record** — this leg was added 2026-08-10 because the row's Property covers the policy governing Klas's mail path while its instrument did not read it, and the two ways to lose it are both silent: STRATO's DMARC control set to "Ingen" deletes the policy, and a second `_dmarc` record makes DMARC not apply at all per RFC 7489 §6.6.3. `email-ses-setup.md` §4 owns the mechanism), and `nslookup -type=TXT jobbliggaren.se 8.8.8.8` (**still no TXT at all** — deliberate for this lane: an apex SPF record governs only the STRATO envelope path and never SES's, so the SES work has no business there, `email-ses-setup.md` §5; whether to publish STRATO's own record for the ordinary mail path is escalated to Klas, not repaired here). Run **after** every DNS change, not once | | |
