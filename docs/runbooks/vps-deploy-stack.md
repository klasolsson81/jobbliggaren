...
⚠ **AND THE PREDICATE NO LONGER HOLDS THE GATE — SAY IT PLAINLY RATHER THAN LEANING ON IT.**
*"Rows 21–25 carry measurements"* is now **literally satisfied by all five**: row 24 carries one.
The objection to 24 is about that measurement's **interpretation**, and the predicate does not
reach interpretation. Before 2026-08-16 the wording did the work on its own — 22 carried a
*negative* measurement and 23 carried *half*, so the gate held by reading. It no longer does.

**DO NOT CLOSE B-1 UNTIL ROW 24'S INFERENCE IS SETTLED OR KLAS ACCEPTS IT.** That imperative
replaces the one this section used to carry (*"do not close B-1 on row 23 alone"*), which the
same visit made obsolete. Closing B-1 releases the 51 347 recruiter contact records, nothing
mechanical enforces the reading, and this paragraph is the enforcement — now a decision to be
taken rather than a condition to be read off. **§5's closing notes carry the same correction; if
the two ever disagree again, that disagreement is itself the defect.** (The heading spans 21–27 because 26 and
...
- **Gate B-1's cutover — and the corpus still waits for it.** #198 shipped the repair (see §2
  ...
  *"rows 21–25 carry measurements"* — is now **literally satisfied by all five**, because row 24
  does carry one; the reservation against 24 is about that measurement's **interpretation** (`139`
  is SIGSEGV, so the crash being the *designed* refusal is inferred from correlation) and the
  predicate does not reach interpretation. So the gate no longer holds on its own text, and what
  holds it is judgement: **do not close B-1 until row 24's inference is settled or Klas accepts
  it.** That is a weaker instrument than a failing row and it is stated as such rather than
  dressed up — the enforcement here is now a decision to be taken, not a condition to be read off.
  A negative measurement satisfies "carries a measurement" and does not satisfy the gate; so does
  an inferred one, and that is the residual.** **Klas confirmed the sequencing
