...
| 27c | **The chosen target is EU, unversioned, and its 30-day rule is the PROVIDER's** | `aws s3api get-bucket-location` / `get-bucket-versioning` / `get-object-lock-configuration` / `get-bucket-lifecycle-configuration` against the live container | **Measured, all four.** OVHcloud Object Storage, container `jobbliggaren-backups`. Location `eu-west-par` (Paris, EU). Versioning **not enabled** and Object Lock **not enabled** — Klas's decision, and the branch ADR 0125 names as strictly stronger for the one-generation property; the cost is that Object Lock is creation-time and is now permanently closed on this container. Lifecycle `k4-main-artefacts-30-days`, `Expiration: 30 days`, **`Filter.Prefix: main/`** — scoped deliberately, because a time expiry over `deks/` would expire the KEYS 30 days after their last write and leave younger main artefacts permanently unreadable | 2026-08-09 |
| 33 | **The upload credential cannot DELETE — and it CAN, so this row is a GATE and it is OPEN** | `aws s3api delete-object` with the box's credential; then `get-bucket-policy` and `get-bucket-acl` to establish whether a policy could repair it | **Measured FALSE 2026-08-09, and the reason is structural.** `delete-object` **succeeded**. `get-bucket-policy` → `NotImplemented` on OVH, so the S3 instrument does not exist here; `get-bucket-acl` → the backup user **OWNS** the container with `FULL_CONTROL`, and OVH's documented evaluation authorises an owner even with no explicit allow. ADR 0125 Decision §3's credential-without-`DELETE` — the property that chose OVH over Hetzner Storage Box — is therefore **not in force**, and no policy on this container can put it there. **Repair is Klas's and is structural: the container must be owned by an identity the box does not hold**, with the backup user granted a policy omitting `s3:DeleteObject`. A compromised box can destroy the whole backup history until then. Blocks neither backups nor the drill; **it is a first-real-data gate** | | |
