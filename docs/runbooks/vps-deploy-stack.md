...
- **Closed outright — 21, 25.** Property, instrument and measurement agree, no caveat.
- **Property closed, rationale short — 22.** The three instruments do discharge *"no plaintext
  copy on persistent disk"*. What is short is the row's own **rationale**, which turns on a
  rotation: `FieldEncryption__LocalMasterKeyId` reads `local-v1`, the injection script's own
  default, which is evidence **against** one having happened.
- **Vacuously true, Property NOT attested — 27.** The counts are zero, so nothing is locked to
  any pepper. But this row's Property says the peppers were *replaced*, and that is exactly what
  `local-v1` argues against. **A different state from row 22**, and reading the two as the same
  caveat twice is the error to avoid.
- **Half measured — 23** (second half needs data this box has none of) **and 24** (precondition
  established, drill unrun).
- **Gate re-done — 26.** It expired at injection by its own terms and was re-confirmed against
  the injected generation. Its cell states what that re-confirmation does and does not settle.

**B-1 is therefore still open, and its predicate is not this list.** §5's closing notes state it:
B-1 is discharged when rows **21–25** carry measurements. After this pass 21, 22 and 25 do; 23 carries
half and 24 a precondition. (The heading spans 21–27 because 26 and 27 are the gate's *subject*
...
| 38 | **RETIRED IN THIS FORM — the work is UNFINISHED and belongs to the flip, not to this block.** DMARC aggregate reports actually arrive, which the record's presence does not establish | Publish `rua=mailto:dmarc@jobbliggaren.se`, then confirm a report is received within ~48 h. **Blocked on the same STRATO email package as the apex-MX leg of row 36**: the mailbox does not exist yet (Klas, ~1 week from 2026-08-15). The address **must** be on `jobbliggaren.se`: RFC 7489 §7.1 makes an off-domain destination conditional on a `<policy-domain>._report._dmarc.<destination>` record, and `jobbliggaren.se._report._dmarc.gmail.com` was measured NXDOMAIN 2026-08-09 — a Gmail `rua=` would look correct and collect nothing | | |
