...
| # | Property (gate) | Instrument | Measured | Date |
|---|---|---|---|---|
| 27b | **The target's lifecycle actually removes objects, measured as an EFFECT and not as a rule** | Provisioning gate, and it is a gate: `get-bucket-versioning`, then the lifecycle configuration read back, then after two nights `list-object-versions --prefix deks/` returning **exactly one version per key**. **Row 27c settled the branch this row was drafted against, and the conditional half is now inert — read it as history, not as work.** On a *versioned* bucket an overwrite deletes nothing and an `Expiration` rule only writes a delete marker, so both prefixes would need `NoncurrentVersionExpiration` (and `main/` additionally `ExpiredObjectDeleteMarker`). **Versioning was measured OFF on this container (27c, 2026-08-09)**, so no noncurrent version and no delete marker can exist, those two rule types would match nothing, and adding them is not the repair — `backup-restore.md` §8 item 6 records the same finding. **What remains live is the effect measurement, and on an unversioned container the version listing cannot make it:** `list-object-versions` returns one `null`-id version per key whatever the lifecycle does, so "exactly one version per key" is true by construction and discriminates nothing. Measure the objects instead — `deks/` holding exactly one generation, and a `main/` listing after 31 days showing the older artefacts actually gone. A rule is a claim; the disappearance is the measurement | | |
...
**These are UNMEASURED except row 35, and are written as a checklist for the DNS publication,
deliberately.** Procedure and rationale: [`email-ses-setup.md`](email-ses-setup.md). The AWS-side
identities exist as of 2026-08-09; what has not happened is the DNS half, which is Klas's at
STRATO. A row without a date is a claim that cannot be told from one that has decayed.

Row 36 is a **control** row and the reason it exists is that the risk in this work is not to the
new sender but to the old one: the domain publishes `p=reject`, and Klas's ordinary mail survives
it today only through STRATO's own DKIM selectors. Row 37 closes the question rows 33–34 cannot:
DNS resolving is not the same as a receiver accepting.
