...
| 20 | Reconcile-pull runs and applies a digest change | `systemctl list-timers` + journal of a real run | Not measured — the unit installs after this merges and one publish on `main` carries an attestation (row 20b explains why that ordering is forced) | |
