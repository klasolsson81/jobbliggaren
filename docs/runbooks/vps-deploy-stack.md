...
**Four of them closed on 2026-08-15 and two did not, and the split is worth reading before the
table.** Closed: **21, 22, 25, 27**. Half-closed and marked as such in its own cell: **23**,
whose second half needs data this box does not have. Still open: **24**, the reboot drill —
and its precondition is now established for the first time, because
`jobbliggaren-secrets-present.timer` was installed and enabled the same day, so the detector
half no longer has to be deferred (the row says as much since #1329). **Two of the closed cells
carry a named limit rather than a clean tick**, and it is the same limit twice: this session
did not perform a rotation and cannot attest one, so rows 22 and 27 prove *absence on disk* and
*nothing locked to any pepper* — never *the values were replaced*. Read those cells, not this
summary, for what each actually rests on.
...
| 22 | No plaintext copy on persistent disk | `sudo grep -rIlF "$K" /opt /etc /root /home /var/lib/jobbliggaren /var/lib/docker/containers` — expect no output. `grep -c '^FIELD_ENCRYPTION_MASTER_KEY=' /opt/jobbliggaren/deploy/.env` — expect `0`, line **deleted** and not blanked. `sudo journalctl --grep "$K" -q \| head -1` — expect empty. **Named scope limits:** `/var/lib/docker/overlay2` is excluded (a box-generated key cannot be in an image layer — `.dockerignore:35-49`); and freed SSD blocks that once held the old `.env` line are unreachable by any grep, **which is why the cutover rotates the key rather than relocating it** — rotation makes those remnants worthless | **All three instruments zero, and the row's own rationale is only PARTLY discharged — the gap is named rather than left to be inferred.** `sudo grep -rIlF "$K" /opt /etc /root /home /var/lib/jobbliggaren /var/lib/docker/containers` → **0 files**. `grep -c '^FIELD_ENCRYPTION_MASTER_KEY=' deploy/.env` → **0**, deleted and not blanked; a name sweep of `.env` for `MASTER\|PEPPER\|KEY` leaves only `POSTGRES_MASTER_PASSWORD`, so none of the four has a residual home there. `sudo journalctl --grep "$K"` → **empty**. ⚠ **What this cell does NOT establish is the rotation the sentence above hangs on.** It measures *absence on disk*; it does not measure that the tmpfs generation differs from the one `.env` used to hold. Those values were injected **2026-08-12** by an earlier session, and by the time this row was measured the `.env` lines were already gone — so the comparison that separates a rotated key from a relocated one **had no counterparty**, and it was discarded rather than reported as a result. What is independently true and does not depend on it: `user_data_keys` = 0 and `identity.AspNetUsers` = 0 (row 27), so a carried-forward key would lock no ciphertext, and the freed-block argument costs nothing today. Klas confirmed 2026-08-15 that the injected generation is the one held in escrow, which is what row 26 needed re-opened at injection | 2026-08-15 |
| 23 | The app boots and decrypts from the file-sourced key | `docker inspect -f '{{.State.Health.Status}}' jobbliggaren-api` → `healthy`. This **is** key evidence rather than a liveness check: `ValidateOnStart` plus the `LocalDataKeyProvider` constructor make an unreadable or invalid key a boot failure, so a healthy api has parsed and validated the file. Then read one encrypted field through the app (any page showing CV or profile data) — the DEK-level check alone cannot catch a re-wrap that generated a fresh DEK | **First half measured, second half BLOCKED on the absence of data rather than deferred by choice.** `docker inspect -f '{{.State.Health.Status}}' jobbliggaren-api` → **`healthy`**, and it has been continuously so for 46 h with the `_FILE` seam live and `.env` holding none of the four — which is the row's own argument that a healthy api has parsed and validated a file-sourced key, since `ValidateOnStart` plus the `LocalDataKeyProvider` constructor make an unreadable or invalid key a boot failure. Worker likewise up on the same mount. **The second half cannot be run today:** it requires reading an encrypted field through the app, and `identity.AspNetUsers` = 0, `job_seekers` = 0, `user_data_keys` = 0 — there is no CV or profile data on this box to read, so the re-wrap-with-a-fresh-DEK case this half exists to catch has no subject. It resolves on the same trigger as rows 2b and 19: a real user. Do not tick this row on the health status alone | 2026-08-15 (first half only) |
