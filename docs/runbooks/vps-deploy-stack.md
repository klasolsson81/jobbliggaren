...
**B-1 IS THEREFORE STILL OPEN, and row 23 is the whole of what remains.** Its predicate is not
this list: §5's closing notes state it, and B-1 is discharged when rows **21–25** carry
measurements. After this pass 21, 22, 24 and 25 do; 23 carries **half**, its second half wanting
an encrypted field read through the app, which needs data this box does not have. A row that
carries half a measurement does not discharge the gate — and the paragraph above is where that is
most easily misread, with four rows newly closed and the caveats mostly gone. (The heading spans 21–27 because 26 and 27 are the gate's *subject*
...
- **Gate B-1's cutover — and the corpus still waits for it.** #198 shipped the repair (see §2
  ...
  first clause is history; the second still binds, and row 23's second half is what it is waiting
  on.** **Klas confirmed the sequencing
