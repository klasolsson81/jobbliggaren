...
DKIM selectors, so the control is owed **after every DNS change** and not once. It is open here
because the 2026-08-10 measurement read `_dmarc` and the apex MX, and this row's instrument is the
two `strato-dkim-*` selectors, the MX and the apex TXT — `_dmarc` is not among them, so that
measurement cannot tick this row without claiming more than it took. Row 37 closes the question rows
33–34 cannot: DNS resolving is not the same as a receiver accepting, and nothing has been sent. Row
38 needs a record that has not been published.
