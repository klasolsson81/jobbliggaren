...
(Klas, 2026-08-16). What that releases is the JobTech recruiter-contact corpus and therefore
`JobTech__IngestEnabled=true` on the worker — `appsettings.Production.json` says so itself:
*"Turning it on is a `JobTech__IngestEnabled=true` override on the worker service, and it belongs
to the same change that closes B-1."*
...
- **Gate B-1's cutover — and the corpus still waits for it.** #198 shipped the repair (see §2
  ...
  **B-1 IS CLOSED (Klas, 2026-08-16).** All five rows carry measurements and the Properties hold.
  ⚠ **What it releases is the JobTech recruiter-contact corpus and `JobTech__IngestEnabled=true`,
  and nothing else.** `company_register` has no encrypted column and holds registered legal
  entities' business data rather than personal data (ADR 0091; sole traders excluded twice), so
  B-1 never gated it — do not read this closure as authorising anything there. **The rows 21–27 index paragraph in §5 carries this
