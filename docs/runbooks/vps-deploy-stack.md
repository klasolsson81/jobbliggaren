...
### Rows 28–32 — gate M-4's own subject (#197)

**These are UNMEASURED and are written as a checklist for the drill, deliberately** — the same
discipline as rows 21–27 above, and for the same reason: `deploy/` gets no integration coverage
from `ci`, so the box is where these are decided. A row without a date is a claim that cannot be
told from one that has decayed.

Rows 28–31 close **gate M-4**. Row 32 is a **gate**, not a report, and it is Klas's — the same
class as row 26, which it inherits.
...
| 34 | SES itself considers the domain verified — the only authority on that question | `aws sesv2 get-email-identity --email-identity jobbliggaren.se --query '{Dkim:DkimAttributes.Status,Verified:VerifiedForSendingStatus}'`. Expect `SUCCESS` / `true`. Until then the identity exists but cannot sign, and every send fails | | |
| 35 | **The sending identity carries no default configuration set** — `release-checklist.md` §2.5 point 1 precondition 4 | `aws sesv2 get-email-identity --email-identity jobbliggaren.se` and confirm no `ConfigurationSetName` key in the response. This is AWS-side state that **no test in the repo can pin**; the request-level absence is pinned by `SesEmailSenderTests`, the identity-level attribute is not. Before the identity existed the precondition held only trivially — that is a different and worthless claim | Absent. Response carried `Policies {}`, `Tags []`, and no `ConfigurationSetName` | 2026-08-09 |
| 36 | **CONTROL — the existing STRATO mail path is untouched by the SES work** | `nslookup -type=TXT strato-dkim-0002._domainkey.jobbliggaren.se 8.8.8.8` and `-0003` (both still `v=DKIM1`), `nslookup -type=MX jobbliggaren.se 8.8.8.8` (still `smtp.rzone.de`), and `nslookup -type=TXT jobbliggaren.se 8.8.8.8` (**still no TXT at all** — the apex SPF absence is a decision, `email-ses-setup.md` §5, not an omission to repair). Run **after** every DNS change, not once | | |
...
- **Backup and restore** — [`backup-restore.md`](backup-restore.md) (#197): the nightly job,
  the split-dump model, retention, and the restore procedure. The **target** is still open —
  ADR 0125 binds a requirement profile (EU, S3-compatible, server-side lifecycle, a credential
  that can exclude `DELETE`, a provider **and account** distinct from Netcup) and Klas selects
  and contracts; the interim target is his workstation, which §7 of ADR 0050's amendment rules
  acceptable only while the box holds no real data. **Escrow of the age private key is the same
  gate as row 26 above and is equally undecided** — see rows 28–32.
