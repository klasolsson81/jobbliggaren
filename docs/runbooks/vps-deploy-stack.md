...
| 31 | **The restore drill (M-4).** A real artefact, decrypted off-box, restored, and read through the application — and the crypto-erasure semantics measured rather than asserted | `backup-restore.md` §5 end to end against a real object from the real target, on a synthetic user manufactured through production paths and then hard-deleted. Record **both counts from step 5**: DEK rows dropped as orphans, and restored users with no key. The second is the crypto-erasure claim, measured. Also record that a non-deleted user's encrypted field decrypts through the app — without that, "unreadable" is indistinguishable from "the restore broke" | | |
