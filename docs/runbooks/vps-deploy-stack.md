...
| 28 | `age` and `rclone` are installable from apt on trixie, and the versions are recorded | `apt-cache policy age rclone`, then `age --version` / `rclone version`. **If either is absent this is a STOPP to security-auditor and Klas, not an improvised binary fetch** — `sops` was measured absent from trixie for #198, so this class of absence is live on this box rather than hypothetical | **Both present in `trixie/main`, so this row's STOPP branch is closed and the `sops` precedent did not repeat.** `apt-cache policy` → `age` candidate `1.2.1-1+b5`, `rclone` candidate `1.60.1+dfsg-4`, both from `debian trixie/main amd64`. Installed, then self-reported: `age --version` → `1.2.1`, `rclone version` → `rclone v1.60.1-DEV`. **Record both spellings, because they disagree and only one of them is a version:** Debian builds rclone without upstream's version metadata, so the binary says `-DEV` while the package is an ordinary release build — a reader checking the tool's own output against an upstream advisory would otherwise read a release as a pre-release and dismiss it. *(Mechanism, not inference: upstream stamps the version at link time via `-ldflags -X github.com/rclone/rclone/fs.Version=…` in its own release workflow, and a build without that flag reports the source default, which carries the `-DEV` suffix — [rclone build.yml](https://github.com/rclone/rclone/blob/master/.github/workflows/build.yml) and [packages.debian.org/trixie/rclone](https://packages.debian.org/trixie/rclone), both read 2026-08-10. Row 28a consumes this version against advisories — it carries the owner, the triggers and the dated reading ([#1289](https://github.com/klasolsson81/jobbliggaren/issues/1289)).)* | 2026-08-10 |
| 28a | **Row 28's versions have an OWNER and a TRIGGER SET, and the reachability argument that makes them safe has a mechanical consumer** ([#1289](https://github.com/klasolsson81/jobbliggaren/issues/1289)) | Advisory side: OSV, keyed on the **package version**, never the binary's self-report — `curl -sS -X POST https://api.osv.dev/v1/query -H 'Content-Type: application/json' -d '{"package":{"name":"github.com/rclone/rclone","ecosystem":"Go"},"version":"1.60.1"}' \| jq -r '[.vulns[].aliases[]? \| select(startswith("CVE"))] \| unique \| length'` (same query with `filippo.io/age` @ `1.2.1` for age). Box side: `systemctl list-unit-files 'jobbliggaren-*'` + `systemctl list-timers` + `systemctl status jobbliggaren-logship.service`. Invocation side: `.github/scripts/rclone-invocation-guard.sh`, blocking in `ci` | **OWNER: Klas** — not a new assignment; `security-auditor` named him in #1289's body (*"CVE research is Klas's task and not hers"*). This row is where that assignment is written down and given triggers. **⚠ NOTHING DETECTS A LAPSE OF THIS ROW AUTOMATICALLY** — the guard watches the repo's invocation surface and is structurally blind to the box; the versions below can drift with `ci` green throughout. **`age` 1.2.1: 0 CVEs**, measured 2026-08-30 on two independent instruments (Debian tracker page: zero CVE references; OSV `filippo.io/age@1.2.1`: zero). **`rclone` 1.60.1: 19 version-exact CVEs, 3 CRITICAL / 4 HIGH.** ⚠ **The Debian tracker OVER-REPORTS and must not be used alone** — it flagged CVE-2026-79775 whose own text reads *"versions >= v1.72.0"*, above ours; OSV is the version-exact instrument. And key any query on the **package's upstream version** — `1.60.1`, the number inside `1.60.1+dfsg-4`, which is what the OSV command above passes, never the binary's `v1.60.1-DEV` self-report — row 28 records why that suffix is a Debian build artefact. **REACHABILITY, graded by `security-auditor` 2026-08-30 (`docs/reviews/2026-08-30-1289-security-auditor.md`): 16 of 19 CLOSED, 3 conditional, and NO CRITICAL and NO HIGH is reachable.** All three CRITICALs need `rclone rcd`; every HIGH needs a subcommand or backend never invoked. Her closing grounds are stronger than a flag absence: `rcat` has **no local write side at all**, no `RCLONE_*` is exported so none can be inherited, object names are `readonly` constants, and `rclone.conf` carries backend options only — it **cannot** set a global flag or start a daemon. **Still conditional, on one box-side read only Klas can take:** CVE-2026-79782 (LOW, `X-Amz-Security-Token` on a same-host HTTPS→HTTP redirect) and CVE-2026-79780 (SSE-C keys, same family) turn on whether the injected `rclone.conf` sets `session_token` / `sse_customer_*`; CVE-2026-71310 (MODERATE) turns on a proxy being set. Until that read, write **"at most three, conditional, LOW/MODERATE"** — never "zero reachable". **⚠ THERE IS NO `apt` REMEDY, AND THIS IS THE SENTENCE THAT SAVES THE NEXT READER A DAY:** trixie ships `1.60.1+dfsg-4`, **sid ships `1.69.3+dfsg-3`**, and the fixes land in `1.74.4`/`1.75.0` (upstream released `v1.75.0` 2026-07-31) — so **no Debian suite carries a patched rclone at all**, and six trixie rows are marked `(no DSA)`, which `vps-base-hardening.md` §7's origin-pinned `unattended-upgrades` (`trixie-security` only) can therefore **never** deliver. If reachability ever fails, the remedy is **an upstream binary off-apt, or dropping rclone for another transport** — both rewrite ADR 0125 and are Klas's call, pre-registered as E1 in `docs/reviews/2026-08-30-1289-cto.md`. **BOX STATE, measured 2026-08-30 ~07:39 CEST, and it corrects row 32:** all backup and logship units **are** installed (`static`); `jobbliggaren-logship.timer` and `-fresh.timer` are **`enabled` and firing hourly** (last trigger 07:18:07 CEST, next 08:17:11); backup timers `disabled`. `rclone` nonetheless does not execute — `jobbliggaren-logship.service` carries `ConditionPathExists=/run/jobbliggaren/host-secrets/Backup__RcloneConfigBase64` and systemd recorded *"start condition unmet … was not met"*, so the unit is **skipped, inactive, not failed**, which is why its journal reads "No entries". ⚠ **Instrument limit, stated rather than implied:** that absence rests on **systemd's own Condition evaluation**, not a directory listing (`/run/jobbliggaren/host-secrets/` was `Permission denied` to the SSH user), and it measures absence **at 07:18:07**, not continuously. **So the exposure is ARMED AND DORMANT, not absent** — one injected credential away from hourly execution. **TRIGGERS — the check is owed at each of these, and T-inject is the one #1289 was actually missing:** **T-inject** — at credential injection, because that is the step that arms hourly execution; pointer placed in `backup-restore.md` §3. **T-token** — the conditional read above becomes available at exactly that moment; until taken it is written *unavailable*, never *clean*. **T-golive** — before first real data (#1289's own `mvp` ground). **T-backstop** — quarterly, covering only the residual case of a box that never reboots, since injection is required after every one; a faster cadence buys no faster repair when the remedy is off-apt in every branch, and only trains the reader to ignore the channel. **T-surface** — mechanically, on any change to the invocation surface: `rclone-invocation-guard.sh` fails the build, and its red message says re-measure and update this row rather than widen the allowlist | **2026-08-30** |
...
rather than obsolete. ⚠ **ONE of its five legs now has an expectation that is measured FALSE — the
apex TXT — and it is recorded here rather than repaired, because that repair is a DNS decision and
DNS is E0, Klas's.** ⚠ **THE MX LEG'S DEFERRAL ENDED 2026-09-04:** Klas moved apex-MX off the
blackhole, so the row's original expectation measures **TRUE** again; the reading is in the Measured
cell. Measured against 8.8.8.8 on 2026-08-15, and kept as the dated record of the window it opened:
the apex MX **was** `blackhole.tem.scaleway.com` and no longer `smtp.rzone.de`, so
`kontakt@jobbliggaren.se` received no mail; and the apex TXT carried `v=spf1 include:_spf.tem.scaleway.com -all` where the row
expects none at all, which put Klas's own STRATO envelope in SPF **hard fail** under
`p=reject`, saved by `strato-dkim-0002`/`-0003` alone. ⚠ **THAT SECOND HALF IS SPENT.** Klas added STRATO's include
on 2026-08-28, so his envelope now passes SPF **and** DKIM and no longer hangs on the two
selectors alone (`security-auditor` Minor 2, discharged). The record itself lives in this row's
Measured cell and is not repeated here. **The
leg itself stays open** — the row still expects no TXT at all, so the expectation is measured
false in a second way. **Read that leg as an open question for E0, never as a
deviation to "repair" toward the old expectation** — and read the three legs (the two
...
| 36 | **CONTROL — the existing STRATO mail path is untouched by the transactional-email work. Provider-independent: its subject is Klas's own outbound business mail, which was never sent by SES and is not sent by Scaleway.** *(Two rows in this block are live, on different grounds: this one because its subject was never the provider, row 35 because its property never was.)* | `nslookup -type=TXT strato-dkim-0002._domainkey.jobbliggaren.se 8.8.8.8` and `-0003` (both still `v=DKIM1`), `nslookup -type=MX jobbliggaren.se 8.8.8.8` (⚠ **expectation measured FALSE 2026-08-15: `blackhole.tem.scaleway.com`, not `smtp.rzone.de`** — `kontakt@jobbliggaren.se` therefore received no mail through that window, and that address is `EmailTemplates.ContactAddress`, the Reply-To on every send and the published Art. 12 contact. Klas deferred this on 2026-08-15 pending the STRATO email package; the escalation schedule has ONE home — `release-checklist.md` §2.5 point 1 leg (e) precondition 5 — and this row cites it rather than restating it. ⚠ **The clause this row carried until 2026-08-16 ("BLOCKER at the first real user or at the flip, whichever comes first") is spent in its second half: the flip HAPPENED on 2026-08-16 and the ruling is NO Blocker.** Read the live schedule there, never here. Do NOT restore the old expectation as a "repair" — record what resolves. ⚠ **This row is not the precondition set and does not reproduce one. Do not move MX on this row alone:** read #183's **apex-MX** leg list in `gdpr-processing-register.md` (gitignored, main checkout only) and confirm every leg first. The list is not restated here, and a leg that looks discharged on this surface has not been read. ⚠ **The order is not cosmetic, and NO mechanism enforces it:** the copy-flip PR carries `automerge` at creation, **`agents-done` withheld**, and **`blocked`** — the label §6.5 defines as a §9.6 STOPP to Klas, and the only one of that section's four diagnoses whose prescribed action is not to clear the state. Without it a withheld `agents-done` is indistinguishable from *armed-but-gated by design*, which every session is told to resolve by waiting the agents in. Intent is not permission (#836), and a PR that merges before the move asserts a processing that is not happening. **Klas moves MX; CC never does.** After the move, the session that ran the panel re-measures this leg, verifies HEAD is unchanged, and only then sets `agents-done`; `blocked` comes off in that same step and never before it. ⚠ **The merge does not close the window** — publication is a separate mechanism with its own home, and what bounds the harm meanwhile is the `basic_auth` block in `deploy/caddy/Caddyfile`, which owns that conclusion), `nslookup -type=TXT _dmarc.jobbliggaren.se 8.8.8.8` (**still carries `p=reject`, and still EXACTLY ONE record**. Read it that way and not as byte-equality: row 38 adds `rua=` through this same control, and §4 states that is purely additive, so a `rua=` present here is row 38 working rather than a deviation. Getting that wrong is the expensive direction — an operator who has just read §4's warning that `_dmarc` is destructive to touch would "repair" the mismatch by deleting `rua=`, silently undoing row 38 with no row going red. This leg was added 2026-08-10 because the row's Property covers the policy governing Klas's mail path while its instrument did not read it, and the two ways to lose that policy are both silent: STRATO's DMARC control set to "Ingen" deletes it, and a second **DMARC** record makes DMARC not apply at all per RFC 7489 §6.6.3, which discards anything not starting with `v=DMARC1` and only then terminates on a surviving set of more than one — hence the two-part expectation. Reading for exactly one TXT record here is stricter than the RFC needs, and deliberately so: on a control row an over-strict test fires when nothing is wrong, never the reverse. The mechanism, kept here because the runbook that owned it is deleted: STRATO's DMARC control set to "Ingen" deletes the record outright, and both losses are silent), and `nslookup -type=TXT jobbliggaren.se 8.8.8.8` (⚠ **expectation measured FALSE: this row expects no TXT at all, and the apex carries one. The record is in the Measured cell and is not restated here.** **SPF is evaluated against the ENVELOPE (MAIL FROM) domain, never the From header** (RFC 7208 §2.4) — that is the whole of why this leg exists, and it is the sentence the deleted runbook's §5 carried. Klas's ordinary STRATO mail has an envelope on `@jobbliggaren.se`, so it IS governed by this record) | **Five legs, all read against 8.8.8.8 in one pass.** `strato-dkim-0002` = `v=DKIM1; k=rsa` and `strato-dkim-0003` = `v=DKIM1; k=ed25519`, both **TXT and not CNAME**, one record each; the key material is public DNS data and is deliberately not reproduced here — record the property, per row 33's precedent. `MX` = **`5 smtp.rzone.de`**, ⚠ **CHANGED 2026-09-04 — Klas moved apex-MX off Scaleway's blackhole**, read against 8.8.8.8 **and** against the authoritative `shades02.rzone.de`, so it is not a cache artefact. The row's original expectation holds again. ⚠ **THIS LEG DOES NOT MEASURE WHETHER THE MAILBOX RECEIVES, and the two must not be run together.** Routing and acceptance are separate facts and only the first is read here; §2.5 precondition 5 names the instrument for the second. **Klas confirmed on 2026-08-28 that sending FROM that mailbox worked while receiving did not, and the two were not in tension:** outbound never consults MX and inbound is routed by nothing else, so a working send proves nothing about the leg this row measures. `_dmarc` = **`v=DMARC1; p=reject`, exactly one TXT record**, still no `rua=` (row 38). Apex `TXT` = **`v=spf1 include:_spf.tem.scaleway.com include:_spf.strato.com -all`**, changed 2026-08-28 by Klas. **All five legs re-read 2026-09-04** after Klas moved apex-MX; only the MX leg's value changed, and the other four are unchanged. **Catch-all half read 2026-09-05 (Klas, from an outside address): a send to a non-existent local part BOUNCED — `550 5.1.2 No such mailbox`, returned by the remote server within twelve seconds. No silent acceptance.** | 2026-09-05 |
...
- **That Trivy gate speaks once, at build time, and a second workflow is what speaks after**
  ...
  **published digests** — what an *unpinned* reconcile pulls — with the same parameters
  ...
  is not yet measured, and the workflow's own header says so.
