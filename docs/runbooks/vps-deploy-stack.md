...
- **22 is now a clean tick, as of 2026-08-16.** ~~Its Property was measured FALSE~~ — its own
  ...
  pattern file's non-emptiness and a positive control both checked first — the two fail-open modes
  this row's Instrument column warns about.
...
- **Inference, not measurement, on one half — 24.** The crash-loop and the detector are measured;
  that the crash is the *designed* refusal rather than an unrelated fault is inferred from
  correlation. `139` is SIGSEGV, and api's log is unrecoverable.
- **Both halves measured — 23**, as of 2026-08-16. ~~Second half needs data this box has none
  of~~ — the [#734](https://github.com/klasolsson81/jobbliggaren/issues/734) visit created the
  first accounts, a cover letter was written and read back through the app, `user_data_keys` went
  **0 → 1** under `cmk_key_id` `local-v3`, and the stored column was confirmed to hold no
  plaintext. That last check is what separates the measurement from an echo of the request.
...
**BOTH ROWS THAT HELD B-1 MOVED ON 2026-08-16 — AND B-1 IS NOT THEREBY CLOSED.** Its predicate is
...
**What remains is 24, and it is a weaker objection than the two it replaces — but it is not
nothing.** Its Property is *"reboot survival is the DESIGNED failure"*. Self-heal and the detector
are measured; that the crash-loop is the designed refusal rather than an unrelated fault is
**inferred from correlation**, and `139` is SIGSEGV rather than the shape of a managed
`ValidateOnStart` abort. The row owes one line at the next reboot to settle it, and flags that a
reproducible `139` would be a defect in its own right.

**Whether that inference discharges B-1 is not a docs-layer call, and this file does not make it.**
Closing B-1 releases the 51 347 recruiter contact records, and nothing mechanical enforces the
reading — this paragraph is the enforcement. Before 2026-08-16 it named two rows because two rows
were failing; it now names the one that is left, and hands the judgement to Klas rather than
converting "the blockers are gone" into "the gate is open". (The heading spans 21–27 because 26 and
...
**Rows 33–35 are kept as measured history, not as instruments.** They record that the domain was
DKIM-verified at AWS on 2026-08-10 and that the identity carried no configuration set — true when
written, about a provider that is gone. Do not run them, and do not read a green cell as coverage
of anything shipping now. **Rows 37 and 38 were open and are now moot in that form**: row 37's
...
no mail; and the apex TXT is `v=spf1 include:_spf.tem.scaleway.com -all` where the row expects none
at all, which puts Klas's own STRATO envelope in SPF **hard fail** under `p=reject`, saved by
`strato-dkim-0002`/`-0003` alone. **Read those two legs as open questions for E0, never as
...
they are the mechanism carrying his mail today. The row was already open for a second reason that
still stands: the 2026-08-10 measurement covered two of its five legs, and a control row whose
instrument under-reaches its own property reads as coverage it does not have.
...
- **Gate B-1's cutover — and the corpus still waits for it.** #198 shipped the repair (see §2
  ...
  half is unmeasured, and row 22 carries a measurement that says its Property is FALSE (the key
  is in the persistent journal, put there by that row's own instruments —
  [#1343](https://github.com/klasolsson81/jobbliggaren/issues/1343)). A negative measurement
  satisfies "carries a measurement" and does not satisfy the gate.** **Klas confirmed the sequencing
