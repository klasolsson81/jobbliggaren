...
>   alarm surface. But its `--check` is one predicate over two directories and demands the
>   host-only backup credential too, so enabling it before that credential exists fails every
>   fire and lights the alarm surface permanently — which costs more than the window it covers.
>   `master-key-ops.md` §2 carries the ordering and what deferring costs.
>
> Both are the install block in [`master-key-ops.md`](master-key-ops.md) §2. Run it **before**
> the first `docker compose up`, then inject (§3 of that runbook), confirm `--check` passes, and
> only then enable the timer — §2's last step, which sits after the injection rather than in the
> block for the reason stated there. Nothing mechanical enforces this ordering — that is what
> these lines are.
...
| 24 | Reboot survival is the DESIGNED failure, and self-heal works | `sudo systemctl reboot`. Expect: api `restarting` (crash-loop, fail-closed — no fallback key), `jobbliggaren-secrets-present.service` in `systemctl --failed` within ~2 min naming the missing files. **The second half of that expectation presupposes the timer is ENABLED, which §2 makes conditional on #197's `Backup__RcloneConfigBase64` being injectable — so on a box where it was deferred, an empty `systemctl --failed` here measures the deferral and not the detector. Leave the cell empty and re-run this row when the timer is enabled; do not tick it on a silence that means the opposite.** Then inject and expect api `healthy` within one restart-backoff interval **with no `compose up` and no reconcile run**. Closes two unmeasured premises at once: the crash-loop-then-self-heal behaviour, and that the absence detector actually fires | | |
