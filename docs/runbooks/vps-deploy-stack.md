...
> - **Install `jobbliggaren-secrets-present.timer` in the same step — but enable it only once
>   #197's `Backup__RcloneConfigBase64` can be injected.** A crash-looping container never
>   appears in `systemctl --failed`; that timer is what puts the condition on the box's only
>   alarm surface. But its `--check` demands the host-only backup credential too, so enabling it
>   before that credential exists fails every fire and turns that surface into a continuous page.
...
> the first `docker compose up`, then inject (§3 of that runbook), then confirm `--check` reports
> nothing missing **under `/run/jobbliggaren/secrets`** — on a box without the rclone config it
> still exits 1 on the host-only key, and §3 says what that looks like — and only then enable the
> timer. Nothing mechanical enforces this ordering; that is what these lines are.
...
| 24 | Reboot survival is the DESIGNED failure, and self-heal works | `sudo systemctl reboot`. Expect: api `restarting` (crash-loop, fail-closed — no fallback key), `jobbliggaren-secrets-present.service` in `systemctl --failed` within ~2 min naming the missing files. **This row measures two properties and only the second is conditional. The crash-loop-then-self-heal half is measurable today and should be ticked today.** The detector half presupposes the timer is ENABLED, which `master-key-ops.md` §2 makes conditional on #197's `Backup__RcloneConfigBase64` being injectable — so on a box where it was deferred, defer **that half** and note it in the cell, then re-run it when the timer is enabled. And read the right instrument: what proves the detector fired is `jobbliggaren-secrets-present.service` **appearing in** `systemctl --failed`, not the list being non-empty; on a deferred box its absence from the list means the deferral, which is the opposite of what this row asks. Do not tick on that silence. Then inject and expect api `healthy` within one restart-backoff interval **with no `compose up` and no reconcile run**. Closes two unmeasured premises at once: the crash-loop-then-self-heal behaviour, and that the absence detector actually fires | | |
