...
turned three "vacuous" or "short" cells into attested ones. The states now:
...
- **The rotation is what moved 22 and 27**, but neither is a clean tick.
  `FieldEncryption__LocalMasterKeyId` now reads **`local-v2`**, a value the script's
  `DEFAULT_KEY_ID` cannot produce — precisely the discriminator `local-v1` failed to be. So 22's
  rationale is no longer short. **But 22's Property is measured FALSE on this box right now**:
  its own instrument set wrote the key into the persistent journal via `sudo`'s command logging,
  and the entries stand until [#1343](https://github.com/klasolsson81/jobbliggaren/issues/1343)
  is decided. And **27 is operator-attested, not measured** — the identity binds to the master
...
- **Half measured — 23** (second half needs data this box has none of).
...
**B-1 IS THEREFORE STILL OPEN, AND TWO ROWS HOLD IT — 22 AND 23.** Its predicate is §5's closing
notes: B-1 is discharged when rows **21–25** carry measurements. Read that predicate carefully,
because **"carries a measurement" is satisfied by a NEGATIVE one and the gate is not.** Row 22
carries a measurement and that measurement says its Property is **false**: the master key is in
the box's persistent journal, put there by row 22's own instruments, and it stays there until
[#1343](https://github.com/klasolsson81/jobbliggaren/issues/1343) is decided. Row 23 carries
**half** — its second half wants an encrypted field read through the app, and this box has no
data. So: 21, 24 and 25 discharge; 22 is measured-and-failing; 23 is half. **Do not close B-1 on
row 23 alone.** Closing it releases the 51 347 recruiter contact records, and nothing mechanical
enforces the reading — this paragraph is the enforcement, which is why it names both rows rather
than the more memorable one. (The heading spans 21–27 because 26 and 27 are the gate's *subject*
while 21–25 are what discharge it — two ranges answering different questions, not a typo.)
