...
the 2026-08-10 measurement covered **one of its four legs** — the apex MX — while the two
`strato-dkim-*` selectors and the apex TXT went unread. (That measurement also read `_dmarc`, which
is not one of this row's legs at all, so it adds nothing here; a surplus reading cannot substitute
for a missing one.) Row 37 closes the question rows 33–34 cannot: DNS resolving is not the same as a
