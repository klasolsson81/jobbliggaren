...
> **Version:** 1.11
> **Senast uppdaterad:** 2026-05-10 (STEG 14a APPLIED — GitHub OIDC + workflows committade, build.yml mekaniskt verifierad)
...
|------|-----|-------------|--------|
| STEG 14b | Fas 0 | **DDL-init + ConnectionStrings split + Worker-recovery** (2 av 3 14-block). Investigate + fix TD-37 (Integration-tests fail i CI). DDL via AWS Session Manager port-forward → Hangfire `Install.sql` + `CREATE ROLE jobbpilot_migrations/jobbpilot_app/jobbpilot_worker` + REVOKE PUBLIC + ALTER DEFAULT PRIVILEGES per `docs/runbooks/hangfire-schema.md §3-4`. `aws secretsmanager put-secret-value` × 2 (db_app_connection + db_hangfire_connection). ECS force-new-deployment → Worker stable + Hangfire-jobs running (cron 03:00/03:30/04:00 UTC). | Planerad |
| STEG 14c | Fas 0 | **First formal tag-deploy + Bootstrap-cleanup + Fas 0-stängning** (3 av 3 14-block). Verify Bootstrap-IAM-user borta (redan tom enligt 14a-discovery — bara dokumentations-step). `git tag v0.1.0-dev && git push origin v0.1.0-dev` → trigger deploy-dev.yml → första formal CI/CD-deploy. Smoke-test mot https://dev.jobbpilot.se/api/ready. ADR (om Fas 0-completion-decision nödvändig). README.md update "Status: Fas 0 → Fas 1". **Stänger Fas 0** per BUILD.md §18. | Planerad |
...
**STEG-fokus:** **STEG 14a APPLIED 2026-05-10** — GitHub OIDC-federation aktiv (provider + dev-deploy-roll i AWS), GitHub Actions workflows (build.yml + deploy-dev.yml) committade och pushade. AWS_DEPLOY_ROLE_ARN GH-secret satt. build.yml mekaniskt verifierad: alla pipeline-steps körs korrekt, frontend (Next.js 16) helgrön, backend Domain/Application/Architecture-tests gröna. Backend Integration-tests fail (88 errors på Api + 1 på Worker) — **förexisterande issue** som CI exponerar (lokalt passar 554/554). Lyft som TD-37, addresseras vid 14b. **Nästa:** STEG 14b (DDL-init + ConnectionStrings split + Worker-recovery + TD-37-fix) sedan STEG 14c (first formal tag-deploy + Fas 0-stängning).
