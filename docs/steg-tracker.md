...
> **Version:** 1.9
> **Senast uppdaterad:** 2026-05-09 (STEG 13a kod-skriven, ej applied)
...
|------|-----|-------------|--------|
| STEG 13b | Fas 0 | **Container-infra + edge** (Alt A2 andra block). ECR repos (jobbpilot-api, jobbpilot-worker) + lifecycle policies, Dockerfiles (multi-stage .NET 10, non-root, healthcheck), ECS Fargate cluster + task-definitioner + services (Api 2-10 tasks, Worker 1-4 tasks), ALB + listeners (HTTPS via ACM) + target groups + health checks, Route53 zone + ACM-cert för dev.jobbpilot.se, CloudWatch LogGroups med retention=30 (ADR 0024 D7), IAM execution-roles + task-roles (Bedrock-policy attach + Secrets Manager get + KMS Decrypt + ECS Exec), KnownNetworks-overlay-värde i task-def (`10.0.0.0/16`). Förutsätter STEG 13a applied. ~1-1.5 session. security-auditor + dotnet-architect i loop. | Planerad |
...
**STEG-fokus:** STEG 13a kod-skriven 2026-05-09 (ej applied). Tre nya Terraform-modules (`network`, `rds`, `redis`) + ny `environments/dev/`-stack komponeras via baseline-master-KMS-key-lookup. Sec-Major-1 + Sec-Minor-6 fixade in-block (RDS log-konfig: hindrar password-leak vid DDL + PII via slow-query bind-värden + 30d CloudWatch-retention). Sec-Major-2 ADR-accepterad via ADR 0025 (ECS-egress 0.0.0.0/0 Fas 0-acceptance med förberedd hardening-väg). Inga aktiva STEG. Nästa: operativa pre-apply-steg (SSO-login + budget-höjning + version-verifiering + apply) följt av STEG 13b (container-infra).
