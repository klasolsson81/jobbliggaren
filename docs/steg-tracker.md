...
> **Version:** 1.11
> **Senast uppdaterad:** 2026-05-10 (STEG 14b APPLIED — DDL-init + Worker-recovery COMPLETE, Hangfire BackgroundJobServer running)
...
|------|-----|-------------|--------|
| STEG 14c | Fas 0 | **TD-37 investigation + first formal tag-deploy + Bootstrap-verify + Fas 0-stängning** (3 av 3 14-block). Block 1: TD-37 — reproducera Integration-tests-fail lokalt med Linux-Docker (devcontainer eller `act`), verbose Serilog för 500-error-rot, eventuell isolering till separat workflow-job. Block 2: `git tag v0.1.0-dev && git push origin v0.1.0-dev` → trigger deploy-dev.yml → första formal CI/CD-deploy. Smoke-test mot https://dev.jobbpilot.se/api/ready. Block 3: verify Bootstrap-IAM-user borta (redan tom enligt 14a-discovery — bara dokumentations-step). ADR 0028 (om Fas 0-completion-decision nödvändig). README.md update "Status: Fas 0 → Fas 1". **Stänger Fas 0** per BUILD.md §18. | Planerad |
...
**STEG-fokus:** **STEG 14b APPLIED 2026-05-10** — DDL-init + Worker-recovery COMPLETE. 3 Postgres-roller skapade (jobbpilot_migrations + jobbpilot_app + jobbpilot_worker), Hangfire-schema + 13 tabeller installerade, GRANT-modell aktiv. ConnectionStrings split skrivna till Secrets Manager. Worker + API stable. Hangfire BackgroundJobServer running. Smoke-test PASS. 5 debug-iterationer fångade 4 unika klasser av buggar (pl/pgsql-param-scope, RDS-master limited superuser, Windows charmap, KMS-policy). security-auditor:s Sec-Minor-5 (ta bort kms:GenerateDataKey) **empiriskt motbevisad** vid Phase D. Spend ~$79.65/mån. **Nästa:** STEG 14c (TD-37 investigation + first formal tag-deploy + Bootstrap-verify + Fas 0-stängning).
