...
/// <summary>
/// Self-service change-email — REQUEST step (#679, C5-email of epik #481). The re-authentication
/// credential is a purpose-scoped grant (#1739, ADR 0142 D5): <see cref="IReauthenticatingRequest"/>
/// makes <c>ReauthenticationBehavior</c> redeem it server-side (oracle-safe 401) BEFORE the handler runs,
/// so a hijacked long-lived session cannot repoint the account-recovery vector on its own. The
/// architecture tripwire (<c>ReauthenticationTripwireTests</c>) forces this marker + a validator on any
/// <c>ChangeEmail…</c> command.
///
/// <para>
...
/// on the Identity user, not the JobSeeker aggregate). Neither the grant nor the new email ever
