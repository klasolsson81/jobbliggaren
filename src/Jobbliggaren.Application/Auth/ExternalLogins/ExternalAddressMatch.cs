...
/// ordinal. The four characters Unicode case-folds into ASCII letters are not ASCII, so they never reach the
/// case-insensitive branch and #1779 stays closed. Identity already holds two ASCII spellings that differ only in
/// case as one account (the unique normalised user name), so this cannot join two accounts the system keeps apart.
