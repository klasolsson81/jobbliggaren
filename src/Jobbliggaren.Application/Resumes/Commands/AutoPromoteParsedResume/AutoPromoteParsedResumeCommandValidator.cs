...
/// validation pipeline (CTO-bind 2026-07-17 §2). <c>NameOverride</c> is the CV LABEL and is
/// optional (absent → the handler generates a non-PII default, #1060); when present it obeys
/// the same 200-cap as every CV-name write surface
/// (<c>Resume.Rename</c>/<c>CreateFromParsed</c> parity).
/// </summary>
