...
// it. BOTH arms read here: the endpoint ATTEMPTS auto-promote on every upload and Promote()
// soft-deletes the artifact, so restricting this read to PendingReview made the proposals
// unreachable on the ordinary path. The latest_role fallback cannot stand in for it — that is
// ONE denormalised string, while these proposals are the import's own union over education and
// experience, and a CV whose most recent entry names an employer derives nothing from it.
//
