...
/// <summary>
/// Returns the OWNING job seeker's non-PII SSYK occupation proposals for a PendingReview
/// parsed-CV staging artifact (F4-8). Mirrors <c>GetParsedResumeQueryHandler</c>'s fail-closed
/// IDOR shape EXACTLY (resolve owner → owner-scoped find → cross-user/not-found → null + audit,
/// no enumeration oracle), with ONE deliberate difference: it PROJECTS the plain-jsonb
    ...
    : IQueryHandler<GetParsedResumeOccupationsQuery, IReadOnlyList<OccupationProposalDto>?>
{
        ...
        GetParsedResumeOccupationsQuery query, CancellationToken cancellationToken)
    {
        ...
        if (found is null)
        {
            ...
            // legitimate own-promote (parity GetParsedResumeQueryHandler).
