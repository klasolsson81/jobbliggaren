...
public sealed class ErasedAdReadPathTests(ApiFactory factory)
{
    ...
    public async Task GetMyMatches_drops_an_erased_ad_and_KEEPS_an_active_one()
    {
        ...
        using (scope)
        {
            ...
            db.JobSeekers.Add(JobSeeker.Register(userId, "Erased Read Path", TermsAcceptance.AcceptCurrent(ClockAt(T0)), ClockAt(T0)).Value);
    ...
    public async Task ListSavedJobAds_renders_an_erased_ad_as_the_orphan_row_and_KEEPS_an_archived_one()
    {
        ...
        using (scope)
        {
            var seeker = JobSeeker.Register(userId, "Erased Read Path", TermsAcceptance.AcceptCurrent(ClockAt(T0)), ClockAt(T0)).Value;
