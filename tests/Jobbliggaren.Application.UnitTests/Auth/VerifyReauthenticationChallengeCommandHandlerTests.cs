...
/// <summary>
/// The re-authentication challenge's code arm (#1739, ADR 0142 D5). The handler passes the binding and never
/// compares; a verified code is a grant for the session's user and nothing else; the refusals are the login
/// code's, from the one shared mapping.
/// </summary>
public sealed class VerifyReauthenticationChallengeCommandHandlerTests
{
    ...
    public async Task A_wrong_code_answers_the_login_codes_error_and_mints_nothing(
    ...
    {
        // Another user's challenge, another purpose's and a login challenge all arrive here as Missing: the
        // store answers them so, and this handler tells none of them apart from an expired one.
    ...
    [Fact]
    public void The_code_arm_takes_neither_the_outcome_function_nor_the_session_store()
    {
        // A bound proof must never become a session (ADR 0142 D5). ReauthenticationChainTests pins the consumer
        // lists; this pins the constructor, which is what the CTO bound.
        var parameters = typeof(VerifyReauthenticationChallengeCommandHandler)
            .GetConstructors()
