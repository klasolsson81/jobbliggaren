...
/// <summary>
/// CompanyWatchPseudonymizationOptionsValidator (#544, ADR 0090 D5) — the fail-closed startup guard
/// for the SEPARATE company-watch pepper. Mirrors <see cref="AuditPseudonymizationOptionsValidatorTests"/>
/// 1:1 (security-auditor B1).
/// </summary>
/// <remarks>
/// <b>This validator is the whole defence, and it needs a test.</b> <see cref="HmacProtectedIdentityTokenizer"/>
/// cannot defend itself: HMAC-SHA256 accepts a zero-length key and happily returns a digest, so an
/// absent/weak pepper yields an effectively UNKEYED token of a personnummer-shaped org.nr at rest in
/// <c>company_watches</c>. The personnummer space is small, so an unkeyed token is trivially
/// brute-force-reversible — it would <i>look</i> protected while being strictly worse than plaintext.
...
/// </remarks>
public class CompanyWatchPseudonymizationOptionsValidatorTests
{
    private static CompanyWatchPseudonymizationOptionsValidator Validator() => new();
    ...
    private static Microsoft.Extensions.Options.ValidateOptionsResult Validate(string pepper) =>
        Validator().Validate(null, new CompanyWatchPseudonymizationOptions { PepperBase64 = pepper });
    ...
    /// key length — one number to remember, shared with the audit pepper.
    /// </summary>
    [Fact]
    ...
    /// goes soft, every enskild-firma org.nr tokenised in <c>company_watches</c> silently becomes an
    /// unkeyed, brute-force-reversible token of a personnummer.
    /// </summary>
    [Theory]
    [InlineData("")]
    [InlineData("   ")]
