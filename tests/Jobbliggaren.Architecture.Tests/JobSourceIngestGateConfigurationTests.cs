...
/// the corpus load is authorised at release-checklist.md's corpus item, which is MULTI-CONDITION.
/// Its Art. 28 leg was ticked 2026-08-16 and this guard still stays — NO ONE DISCHARGED CONDITION
/// IS PERMISSION, and the item's state is read there, never restated here. The unit tests
...
public class JobSourceIngestGateConfigurationTests
{
    ...
    public void WorkerProductionOverlay_ShipsIngestDisabled()
    {
        ...
        options.IngestEnabled.ShouldBeFalse(
            "the deployed Worker must not ingest recruiter contact records until the corpus load " +
            "is authorised at release-checklist.md's corpus item. Do NOT read ANY single " +
            "discharged condition as permission — B-1 closed 2026-08-16 and its Art. 28 leg was " +
            "ticked the same day, and this guard was never about either one alone. " +
