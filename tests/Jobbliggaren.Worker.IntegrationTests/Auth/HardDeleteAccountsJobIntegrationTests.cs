...
public class HardDeleteAccountsJobIntegrationTests(WorkerTestFixture fixture)
{
    ...
    public async Task CleanupIdentityOrphans_DoesNotDeleteReverseOrphan_AndForwardSweepStillRuns()
    {
        ...
        using (var scope = _fixture.Services.CreateScope())
        {
            ...
            var seeker = JobSeeker.Register(reverseOrphanUserId, TermsAcceptance.AcceptCurrent(clock), clock).Value;
    ...
    public async Task RunAsync_CascadesHardDelete_ToApplicationResumeAndUserJobAdMatch()
    {
        ...
        //
        // #1681 (ADR 0139; security-auditor Major 5(b), 2026-09-06) — and the criterion's two
        // MATERIALISED tables. Those are NOT FK-less by UserId: they hang off the criterion by a
        // DB-level FK with ON DELETE CASCADE, so nothing in HardDeleteAccountAsync mentions them and
        // AccountHardDeleteCascadeFitnessTests cannot see them either (its partition is over
        // AggregateRoots, and under ADR 0139's Infrastructure-scoped form these are read models).
        // That is exactly why the auditor made this oracle MANDATORY rather than recommended: it is
        // the ONLY thing in the repo that measures whether derived personal data about the user
        // survives her account deletion (Art. 17, Art. 5(1)(e)).
        ...
        using (var scope = _fixture.Services.CreateScope())
        {
            ...
            // EXPLICITLY. The cascade is the only thing that erases it, which is why the C-D1
            // argument needs a behavioural oracle and not just the structural fitness proxy.
            ...
            // TWO criteria, both live. This slot used to hold a SOFT-DELETED second row (a
            // security-auditor Minor 1 from 2026-07-13, guarding "does the cascade see rows the
            // query filter hides?"). That risk is now structurally unreachable rather than untested:
            // delete is HARD (C-D8/G1), no row can be stamped, and the demolition PR removed the
            // method, the column and the filter. The seed is NOT simply dropped — the second row
            // stays, re-aimed at the strongest property still expressible here: the sweep is
            // SET-WIDE. With one row a cascade that erased only the first match would pass; with two
            // it cannot. (The retired mandate's actual successor is the iff-guard in
            // AccountHardDeleteCascadeFitnessTests, which is build-time and covers all eleven arms.)
            var secondCriteria = CompanyWatchCriteriaSpec.Create(["43210"], ["1480"]).Value;
            var secondCriterion = CompanyWatchCriterion.Create(
                userId, secondCriteria, "El i Göteborg", seedClock).Value;
            db.CompanyWatchCriteria.Add(secondCriterion);
            ...
            await db.Database.ExecuteSqlRawAsync(
                ...
                VALUES ('5560000001', 'IT-bolaget AB', '0180', NULL,
                        ARRAY['62010'], false, '1', 'Active', now(), now());
                """, ct);
        }
        ...
        using (var materialiseScope = _fixture.Services.CreateScope())
        {
        ...
        using (var preScope = _fixture.Services.CreateScope())
        {
            ...
            // TWO rows, both live — so the post-condition below proves a SET-WIDE sweep and not just
            // "the first row went". IgnoreQueryFilters is deliberately absent: this aggregate has no
            // query filter to see past (C-D8/G1), and asserting one here would misdescribe the read
            // exactly the way the demolished apparatus did.
            var criterionCount = await preDb.CompanyWatchCriteria
                .AsNoTracking()
                .Where(c => c.UserId == userId).CountAsync(ct);
            criterionCount.ShouldBe(2,
                "seed must persist exactly two company_watch_criteria rows before the job runs — "
                + "two, so a cascade that erased only one would not pass");
        ...
        // #1681 (security-auditor Major 5(b)) — the derived tables go with the criterion, via the
        ...
        DateTimeOffset deletedAt, CancellationToken ct)
    {
        ...
        var seekerResult = JobSeeker.Register(user.Id, TermsAcceptance.AcceptCurrent(registerClock), registerClock);
