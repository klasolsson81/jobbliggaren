 ...
 *
 * Fails (exit 1) when the target CSS contains a hardcoded font-size or color
 ...
 *
 * Usage: node scripts/guard-css.mjs <path-to-css> [--json]
...
function inTheme() {
  return stack.some((s) => s.startsWith("@theme"));
}

function checkDecl(decl, lineNo) {
  ...
  if (prop === "font-size" && hasRawNumber(value)) {
    findings.push({ line: lineNo, selector, decl: `${prop}: ${value}`, rule: "font-size-literal" });
  }
  if (COLOR_PROPS.test(prop) && COLOR_LITERAL.test(value)) {
    findings.push({ line: lineNo, selector, decl: `${prop}: ${value}`, rule: "color-literal" });
  }
}
...
if (asJson) {
  ...
  for (const f of findings)
    console.log(`${String(f.line).padStart(5)}  [${f.rule}]  ${f.selector}  →  ${f.decl.slice(0, 80)}`);
