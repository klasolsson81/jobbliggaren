...
function makeRequest(query = ""): NextRequest {
  return new NextRequest(`http://localhost/api/cv/${VALID_ID}/preview${query}`);
}
...
describe("GET /api/cv/[id]/preview (render-by-Resume-id PDF-passthrough BFF, TD-112 / #202)", () => {
  ...
  it("200 → strömmar PDF med FÄRSKA headers + Bearer-vidarebefordran mot render-by-Resume-id (?profile=Visual)", async () => {
    const fetchMock = vi
      .fn()
    ...
    expect(url).toBe(
      `http://test-backend/api/v1/resumes/${VALID_ID}/render?profile=Visual`
    );
  ...
  it("saknad profil → upstream använder ?profile=Ats (normalisering, inte tyst korruption)", async () => {
    ...
    expect(url).toBe(
      `http://test-backend/api/v1/resumes/${VALID_ID}/render?profile=Ats`
    );
  });
