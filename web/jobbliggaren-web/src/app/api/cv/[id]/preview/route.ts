 ...
 * GET-spegling av `api/cv/parsed/[parsedId]/preview` — enda skillnaden är att vi
 * keyar på Resume-id och proxar mot backendens render-by-Resume-id-väg
 * (`GET /api/v1/resumes/{id}/render?profile=Ats|Visual`). Den befordrade
 * Resume-griden (ResumeCard) saknar ett parsedId, så den behövde denna route.
 *
 * Klienten GET:ar hit (same-origin, cookie auto-skickas) och vi vidarebefordrar
 * till backend med Bearer-auth. Backend returnerar en RÅ PDF (Results.File, inte
 * JSON) — vi strömmar igenom `backendRes.body` direkt (ingen server-side-buffring,
 * ingen klientbunt).
 *
 * PDF-bytesen är ägar-scopade i backend (RequireAuthorization, owner-scoped,
 * IDOR → 404) och persisteras aldrig (Invariant 3).
 *
 ...
 * parsed-routen); backend är den auktoritativa validatorn oavsett.
 */
...
export async function GET(
  ...
  // (samma default som parsed-routen). Backend är auktoritativ.
