...
describe("ForetagSokFollowAll", () => {
  it("enables the CTA and shows the ready explainer for a criterion-shaped filter", () => {
    ...
    const button = screen.getByRole("button", { name: "Spara som smart bevakning" });
    expect(button).not.toHaveAttribute("aria-disabled");
    expect(
      screen.getByText(/Spara branscherna och kommunerna som en smart bevakning/i),
    ).toBeInTheDocument();
  });

  it("disables the CTA with the empty-filter explainer on a browse-all filter", () => {
    render(<ForetagSokFollowAll namn="" sni={[]} kommun={[]} />);
    expect(screen.getByRole("button", { name: "Spara som smart bevakning" })).toHaveAttribute(
      "aria-disabled",
      "true",
    );
    expect(
      screen.getByText(
        "Välj minst en bransch och en kommun för att spara sökningen som smart bevakning.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("disables the CTA with the name-term explainer when a name is present (silent-drift guard)", () => {
    ...
    expect(
      screen.getByText(/En sökning på företagsnamn går inte att spara som smart bevakning/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows the SNI-missing explainer for a kommun-only filter", () => {
    ...
    expect(
      screen.getByText("Lägg till minst en bransch för att spara sökningen som smart bevakning."),
    ).toBeInTheDocument();
  });
  ...
  it("shows the kommun-missing explainer for an SNI-only filter", () => {
    ...
    expect(
      screen.getByText("Lägg till minst en kommun för att spara sökningen som smart bevakning."),
    ).toBeInTheDocument();
  });
  ...
  it("saves the active axes and shows a confirmation linking to the hub on success", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Spara som smart bevakning" }));
    ...
    expect(
      await screen.findByText("Sökningen sparades som smart bevakning."),
    ).toBeInTheDocument();
    ...
    expect(document.activeElement).toHaveTextContent("Sökningen sparades som smart bevakning.");
    // The button is replaced by the confirmation (no accidental duplicate on a second click).
    expect(
      screen.queryByRole("button", { name: "Spara som smart bevakning" }),
  ...
  it("surfaces the action error and keeps the button on failure", async () => {
    ...
    expect(screen.getByRole("button", { name: "Spara som smart bevakning" })).toBeInTheDocument();
  });
