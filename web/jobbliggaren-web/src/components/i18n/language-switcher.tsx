 ...
 *
 * Two consumers, one presentation. The `variant` prop is gone: neither of its
 * members had a live mount left after this change.
 *
 ...
 * change here.
 *
 * A11y: below the width where the name is hidden the accessible name is exactly
 * "Språk" — `display: none` removes a node from the accessibility tree, so the
 * language itself is NOT announced there. That is sound (2.5.3 applies only where
 * a visible text label exists) but it is not the same promise as the wide state.
 *
 * `DropdownMenuRadioItem` gives `role="menuitemradio"` + `aria-checked` —
...
export function LanguageSwitcher() {
  ...
  return (
    <DropdownMenu>
      {/* `aria-disabled`, never `disabled`: React holds `isPending` true through the
          whole async transition, and Radix returns focus to the trigger when the menu
          closes — a disabled trigger drops that focus to <body>. The repo measured
          exactly this for the same control in settings (#1391). `select()`'s own
          guard is what prevents re-entry; `disabled` only cost the focus. */}
      <DropdownMenuTrigger
      ...
      >
        ...
        <span className="jp-langname">{t(active)}</span>
        <ChevronDown size={14} className="jp-langchevron" aria-hidden="true" />
      </DropdownMenuTrigger>
