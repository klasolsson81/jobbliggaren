...
export interface MatchChipProps {
  grade: MatchGrade;
  /** Lets a card name the chip in its link's `aria-describedby`. */
  id?: string;
}

export function MatchChip({ grade, id }: MatchChipProps) {
  ...
  return (
    <span id={id} className={`jp-matchchip ${modifier}`} data-tag="match">
